...
- `tests/test_coverage_gap_mapper_drift.py` -- Dogfood/drift gate (E-4 + C-0 `--enforce`) for the `juniper-coverage-gap-map` console script in `juniper-ci-tools`. Asserts the script is registered, both module halves ship, `_version.py` matches `[project].version`, pins admit it, the `--enforce`/`--fail-under-*`/`--omit` flags are wired, and that `--enforce` over a synthetic `coverage.json` exits 1 on a gap / 0 clean. Behaviour matrix: `juniper-ci-tools/tests/test_coverage_gap_mapper.py`.
