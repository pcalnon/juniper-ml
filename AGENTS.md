...
- `tests/test_coverage_gap_mapper_drift.py` -- Dogfood/drift gate (E-4 + the C-0 enforcing mode) for the `juniper-coverage-gap-map` console script (the per-file coverage-gap mapper in `juniper-ci-tools`). Modeled on `test_ci_tools_drift.py`. STRUCTURAL: asserts the script is registered, both module halves ship, `_version.py` matches `[project].version`, juniper-ml's pins admit it, and the opt-in `--enforce`/`--fail-under-*`/`--omit` flags are wired. END-TO-END (C-0): invokes the shipped entry over a synthetic `coverage.json` (primary JSON path; deterministic in CI) and asserts `--enforce` exits 1 on a gap / 0 clean, 0 on `--help`, and 0 for the advisory default. The cross-repo real coverage run stays a documented manual-verify step; the full behavioural matrix is gated by `juniper-ci-tools/tests/test_coverage_gap_mapper.py`.
