...
- `tests/test_soak_handoff_consensus_checks.py` -- Hermetic gate for `util/ad-hoc/2026-09-04_soak_handoff_consensus_checks.py` (ml#1694). A reducer that keys mutations on its own `obs_id` is a silent no-op that still prints a plausible report; matching only `via-search-output` undercounts the prose form. Also pins the one-way `source-recovered` rescore, the 2026-08-31 pre/post split, and Wilson 26/43 < 0.75 as the CI-red boundary. Synthetic JSONL only.
