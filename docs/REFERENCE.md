...
`util/experiment_stack.bash` + `util/experiments/run_experiment.py` are the **per-run** CLI experimentation tooling (plan Wave 2.1–2.6; this section is Wave 2.7). They bring up a throwaway juniper-data instance plus **cascor and/or recurrence** (never canopy), drive a single experiment YAML against that stack, and write plots/stats/manifest under a durable `RUN_DIR`. How to read the Wave 2.6 `stats.json` / `summary.md` pair (de-ratified `wall_seconds`, per-poll step-duration honesty, `scrape_confirmed` tri-state): [Experiment Stats Summary (SS8.3)](#experiment-stats-summary-ss83).
