...
- `tests/test_soak_next_probe_split.py` -- Complementary gate for `util/soak_next_probe.py`'s §15.4 date split. `tests/test_soak_next_probe.py` pins unprimed stdout against the live ledger and never calls `post_intervention()`; ml#1699 pins the ad-hoc consensus reducer; ml#1700 pins Wilson power. This suite drives synthetic rows and pins that pre-intervention observations do not count as coverage, on-cutoff is POST, `--status` totals are post-only, a pre-intervention pile does not steal the next billed session from an untouched probe, and bare `--probe-id P19` is rejected. Hermetic -- never launches `claude`, never reads the live corpus except the frozen probe registry for the prefix-reject CLI arm.
