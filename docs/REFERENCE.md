...
- `tests/test_soak_run_probe_launch_guards.py` -- Hermetic gate for the unattended launch / capture path in `util/soak_run_probe.py`. Complementary to `tests/test_soak_run_probe.py` (retrieval channel + parse_events): that suite never resolves the `claude` binary, never writes a reaper pidfile, never scrubs the child env, and never builds a TIMEOUT status. Pins that `resolve_claude` falls back to `~/.local/bin` then `/usr/local/bin` (a bare name raises FileNotFoundError under systemd/cron PATH), that `reaper_guard_path` is under `$JUNIPER_EXP_RUN_ROOT/soak-probes` and never `reports/soak/runs/` (the first version granted zero protection), that `probe_child_env` drops a stale `ANTHROPIC_API_KEY`, and that a TIMEOUT status keeps `stderr_tail` (an earlier version bound `err` and dropped it). `util/` is outside every pre-commit Python hook, so this suite is the gate.
