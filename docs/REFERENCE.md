...
- `tests/test_x7_offload_census.py` -- Hermetic gate for the X7 slice-1a off-loop census (`util/ad-hoc/2026-09-04_x7_offload_census_v2.py`; `util/ad-hoc` is outside every pre-commit Python hook).
  Pins the v0.3.0 site-local exemption: offloading `backend.get_status` at one site must not hide another `backend.get_status()` in a different handler (the module-global expression skip that reported 39 where 54 is true, including the three health endpoints X7 is defined by).
  Also pins provenance (an `httpx.AsyncClient` bound as `client` is not blocking; the same name from `get_redis_client` is), UNRESOLVED-vs-helper filtering, CASCOR/LOCAL buckets, and that v1 remains the unsound name-matching negative example.
