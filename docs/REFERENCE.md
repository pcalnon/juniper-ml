...
**Known limitation (degraded no-App path only):** on the fallback path (`RELEASE_TRAIN_APP_ID` unset), a PR opened with the built-in `GITHUB_TOKEN` does **not** trigger CI workflows (GitHub's recursion guard), so a proposal PR shows **no checks** until the owner re-triggers them — close and reopen the PR, or push an empty commit. On the same token path, juniper-ml's 18 lockfile PRs (census 2026-09-22) all ran zero jobs at opening, in two shapes: 5 got no `pull_request` run, and 13 had their runs created and parked at `action_required`. For a parked PR, re-running those runs is the cheapest re-trigger, and 12 of the 13 were released that way.
