...
python3 -m unittest -v tests/test_push_signed_commit.py
...
python3 -m unittest -v tests/test_thread_width.py
...
- `tests/test_service_fork_drift.py` -- Drift gate for the security guards that must hold identically in `juniper-data`'s and `juniper-cascor`'s forks of the `juniper-service-core` middleware/security code, and in `juniper-canopy`'s own `APIKeyAuth` for the two key-handling guards (defect-register §2.3 "Copy drift"; canopy since `APD-ECO-008`). The shared package's own `security.py` is a site of those two guards too (owner ruling 2026-09-23; `juniper-recurrence` imports it). It is read from THIS checkout, never from `<ecosystem root>/juniper-ml` (in a session worktree that is the main checkout), and `SharedPackageGuardTest` asserts it on every run, with no sibling needed. The ecosystem root is located by the two original forks only (`_ROOT_ANCHOR_REPOS`), so a missing canopy checkout FAILS the file-existence test instead of silently skipping every guard — when the test runs. In `docs-full-check.yml` the cross-repo link check runs first and fails on a missing canopy or cascor checkout, which juniper-ml's notes link into, and the drift step has no `if: always()`, so there the step is skipped instead. A missing juniper-data checkout is the silent case: no juniper-ml note links into juniper-data, so the link check passes, and the drift step finds no ecosystem root and skips every sibling site, green (`OK (skipped=3)`; `APD-ML-008` in the defect register). And the guards match marker TEXT: a regression that keeps its markers passes (`APD-ML-008` in the defect register).
  ...
  - Cross-repo assertions gate exactly like `test_ci_tools_drift.py` (`GITHUB_ACTIONS=true` or `JUNIPER_DRIFT_TEST_FORCE_LOCAL=1`); the registry-structure checks and the matcher's negative controls always run. It bites in `docs-full-check.yml`, the only job that runs it with the siblings cloned (`release-train.yml` clones them too, for its own steps; `ci.yml` runs it without them, so only the always-on checks and `SharedPackageGuardTest` bite there).
  ...
  - **No `KNOWN_GAP` rows currently remain** -- all seven copy-drift guards are `ENFORCED` (six until juniper-ml#1974 added `nonshortcircuit-key-compare`). The `KNOWN_GAP` machinery is retained for the next row that acquires a reference implementation; note that with zero such rows `test_known_gaps_are_still_open_or_get_promoted` iterates an empty set and passes **vacuously**.
...
- `tests/test_publish_env_policy_drift.py` -- Drift gate for the **tag-only deployment ref policy** on every `pypi` / `testpypi` environment ([publish-path design](../notes/JUNIPER_2026-08-17_JUNIPER-ECOSYSTEM_PUBLISH-PATH-AUTHORIZATION-DESIGN.md) §6 Option A / §12.5). Since 2026-09-23 it also covers the five `dockerhub` environments, with their own repo set and tag set (`v*`, `juniper-*-v*`). It asserts that no `DOCKERHUB_*` name sits at repository scope (names only; `gh --jq` drops the values) and that no other repo has the environment ([Docker Hub registration procedure](../notes/JUNIPER_2026-09-22_JUNIPER-ECOSYSTEM_DOCKERHUB-SECRET-REGISTRATION-PROCEDURE.md) §6). The `dockerhub` half is a **local** gate (`JUNIPER_DRIFT_TEST_FORCE_LOCAL=1`), because per-PR CI cannot read the image repos.
...
- `tests/test_push_signed_commit.py` -- Tests for `util/push_signed_commit.py` (one signed commit onto an EXISTING branch). Hermetic: `gh` is a Python PATH stub that replays a JSON-configured GitHub state, lets a commit MOVE the branch head, and **enforces `expectedHeadOid` the way GitHub does** -- a mutation whose pin is not the current head is rejected as stale. That enforcement is what separates "sends the pinned sha" from "sends a fresh read"; an unexpected `gh` call exits 99, so a new call cannot pass silently.
  - Pins: `--expected-head` required and FULL (7/12/39-char abbreviations exit 2 before any `gh` call); `expectedHeadOid` == the pin; a push landing between the pre-flight read and the commit is refused, not clobbered; the tool carries no mutation of its own (it calls `open_signed_pr.create_signed_commit`).
  - Refusals exit 1 and write nothing: missing branch (a 404 only -- a 502 is exit 2, never "does not exist"), the default branch, a moved head (also under `--dry-run`). The read-back fails the run (exit 2) on a head that did not move, an unverified commit, a parent other than the pin, a blob sha that differs from the bytes sent, and a deletion that did not happen.
...
- `tests/test_ci_tools_drift.py` -- Lint test (dep-docs plan §5.1) for `juniper-ci-tools` pins. Mirrors `test_doc_tools_drift.py`: globs **every** `.github/workflows/*.yml` in juniper-ml and in each of the **8** cloned consumer repos, extracts every `juniper-ci-tools>=X,<Y` pin, and asserts each range still admits current (read from `juniper-ci-tools/pyproject.toml`). Separately, the four lanes in `_ML_REQUIRED_PIN_WORKFLOWS` (`ci.yml`, `main-verify.yml`, `lockfile-update.yml`, `docs-full-check.yml`) must each still carry *a* pin -- that closed list is the anti-revert guard, not the lint scope. Same skip semantics + `JUNIPER_DRIFT_TEST_FORCE_LOCAL=1` override as the doc-tools sibling.
  - **Scope history (2026-09-11)**: it used to read exactly one file per consumer repo, `<repo>/.github/workflows/ci.yml`, over a six-repo list. That unit did not match the identity it guarded, and two defects lived in the gap: juniper-cascor's `ci-cascor-model.yml` and `ci-protocol.yml` sat at `>=0.6.0,<0.7.0` for three minors (cascor#646), and juniper-recurrence -- which has no `ci.yml` at all, being a monorepo of `ci-recurrence-{app,client,model}.yml` -- was not in the repo list, so none of its 6 pins were ever checked. Coverage went 25/54 live pins to 54/54. `RepoWidePinScanTest` encodes both shapes as synthetic-tree regressions, including the assertion that the *narrow* read is green on the tree the wide read fails.
  - Also carries the **sequence-safety anti-resurrection gate** (rollout W3 / plan §W3 step 3.3): `SequenceSafetyPackageMigrationTest` asserts juniper-ml's tree has no resurrected inline `util/sequence_safety/` copy or the two moved screen tests (a synthetic-fixture negative proves it bites); `main-verify.yml` in the scanned workflows enforces the two new `>=0.9.0,<0.10.0` screen pins still admit current.
...
- `tests/test_thread_width.py` -- `util/thread_width.py`, the shared thread-width helper that turns the perf lane's twice-failed prose warning into a CI gate. Pins the `/proc/self/maps` parser (GNU / Intel / LLVM runtimes including a manylinux-renamed `libgomp-<hash>.so.1`; rejects `libompd`, offload plugins and a directory merely named `libgomp`; de-duplicates segments; keeps a `(deleted)` runtime); that `OpenMPRuntime` raises before `dlopen` on zero / two / unmapped runtimes and otherwise opens `RTLD_NOLOAD`; that the getter guard raises off the main thread **without calling through**, passes main-thread and `torch.*`-internal calls, installs once and never clobbers a later replacement. Then an AST scan of `util/ad-hoc/` and `util/experiments/`: no literal-soname `CDLL`/`LoadLibrary` of an OpenMP runtime, and no `get_num_threads` reference in a file that does not install the guard. Six pre-helper instruments are grandfathered by name and must each still offend, so the list only shrinks; negative controls pin that docstring prose is not a call and that an aliased `raw = torch.get_num_threads` is.
...
- `util/safe_merge.py` -- Merge gate (synthesis R4): merges a PR **only** after its REQUIRED checks have finished green, refusing otherwise. Closes the measured failure where **12% of freshness-synced PRs merged before the re-test they had just paid for could finish**.
  ...
  - **CI budget is PER-REPO** (`REPO_TIMEOUTS`, `timeout_for()`), across *all required contexts on one head*, not one workflow. The prior single `900 s` was derived from ml's `ci.yml` median alone and sat at **canopy's median**, so roughly half of canopy's healthy merges would have refused with "checks did not finish".
    **Re-measured fleet-wide 2026-09-22, n=30 merged heads, required contexts only, each head's FIRST PASS over healthy heads** (`util/ad-hoc/2026-09-22_ci_budget_handoff_reprobe.py first-pass`; independently written instruments in the 2026-09-22 consensus rounds reproduced every row, juniper-ml's on its own window #1981–#2014): ml p90 910 s / max 2005; data 1651 / 2589; cascor 1717 / 2221; cascor-worker 1576 / 2059; canopy 1798 / 2174; cascor-client 1264 / 1626; data-client 1545 / 2565; deploy 450 / 965; recurrence 1114 / 1666.
    Budgets: ml/cascor 2800, cascor-worker 2400, recurrence 2000, deploy 1400, data/data-client/canopy/cascor-client 3300; `DEFAULT_TIMEOUT` 2400 remains the fallback for a repo not in the table. **Do not quote this row — re-measure.** It carried the 2026-09-08 budgets (ml 1500, recurrence 700) for thirteen days after ml#1851 raised both, and on 2026-09-22 data, data-client and deploy no longer cleared their own max and were raised. Those three maxima were mostly runner queue. The owner ruled on 2026-09-22 to ship the three raises over a recorded dissent; whether a budget should absorb runner queue in general stays an open owner decision, recorded above `REPO_TIMEOUTS`. Four budgets now sit at `TIMEOUT_CEILING`. The record is `REPO_TIMEOUTS` in `util/safe_merge.py` and `MEASURED_SPANS` in `tests/test_safe_merge.py`.
    **Raw v2 inflates on later executions.** It reads check-runs with `filter=latest`, the latest per name within each workflow run, so a re-run attempt or a run started by a later event enters the span: canopy#653 read 33,299 s for a pass that failed at 10:08 UTC and had two jobs re-run at 19:01. `first-pass` takes each required context's first execution instead, and drops no head for a repeat, because dropping heads can only lower a max.
...
- `util/push_signed_commit.py` -- The EXISTING-branch sibling of `util/open_signed_pr.py`: adds ONE GitHub-signed commit to a branch that already exists (a CI fix-up, a review change, a waiver trailer on an open PR), which `open_signed_pr.py` refuses by design. Promoted 2026-09-22 because no existing-branch driver had ever been promoted, so ad-hoc copies kept multiplying under `util/ad-hoc/`; each of those now carries a "Superseded" header line pointing here and is retained as provenance (owner policy 2026-08-25 -- retiring them is an owner decision).
  - `python util/push_signed_commit.py --repo R --branch B --expected-head <FULL 40-char sha> --add LOCAL:REPOPATH [--delete REPOPATH] --message M [--commit-body TEXT | --commit-body-file F] [--owner pcalnon] [--dry-run]`. `--add` / `--delete` are repeatable; at least one is required and no repo path may be named twice. Exit 0 committed and read back clean (or dry-run) / 1 refused, nothing written / 2 hard error, including a commit that landed but did not read back as sent.
  - **The head is PINNED, never read live.** `--expected-head` is the head your edits are based on (`gh pr view N --json headRefOid`), and the mutation's `expectedHeadOid` is that same sha. Most ad-hoc drivers read the head just before committing, which keeps GitHub's guard against a push in the next instant and discards the one that matters -- a push that landed while you were editing, which the whole-file upload then silently reverts.
  - **A short sha is refused before any API call.** GitHub does not expand one: here it would read as a moved branch, and `update-branch` answers `expected_head_sha=<short>` with 422 "expected head sha didn't match current head ref" (observed 2026-09-07 on juniper-ml#1819) -- both look like a lost race when nothing raced.
  - Refuses a missing branch (only a 404 counts; any other failed read is exit 2) and the repo's **default branch**. Reads back at the new commit: head moved to it, `verification.verified`, parent == the pin, each added path's blob sha == the bytes sent, each deleted path 404s. Reuses `open_signed_pr`'s `gh` / `parse_add` / `create_signed_commit`; it carries no copy of the mutation. Tests: `tests/test_push_signed_commit.py`.
- `util/wait_for_checks.py` -- Waits for a PR's **required** status checks to finish, then reports honestly. The shared replacement for the hand-rolled "wait for CI" loops that sessions keep re-writing and keep getting wrong the same two ways. Read-only (only `gh pr view` / `gh api .../rules/...` reads — never merges, updates a branch, pushes, or comments), so any session can run it at any time.
  - `python util/wait_for_checks.py --pr N [--repo juniper-cascor] [--owner pcalnon] [--anchor required|observed] [--fail-fast] [--timeout SECONDS] [--interval 20] [--json] [--verbose]`. Exit **0** all required green / **1** a required check failed (named) / **2** timeout with the still-running and never-reported contexts named / **3** hard error.
  - **Omit `--timeout` and it waits the repo's CI budget** — `timeout_for` in `util/safe_merge.py`, read at call time — and prints `wait budget: <s> (<source>)` on stderr; `--json` carries `timeout` and `timeout_source`. The source names a measured `REPO_TIMEOUTS` row, `safe_merge`'s own default for a repo with no measured budget, or `FALLBACK` to a flat 1800 s when that table cannot be read at all. The flat 1800 s used to be the default outright and sat **below eight of the nine budgets**, so a direct invocation on a healthy canopy / cascor / ml PR could exit **2** while every required context was still running. `safe_merge` always passes `--timeout`, so the merge path was never affected. `TimeoutResolutionTest` pins every row.
    Two cautions. The budget line is on **stderr**, so a caller that parses JSON out of `2>&1` must pass `--timeout` (every in-repo caller does). And budgets now reach 3300 s, near the ~3600 s background-worker lease, so a **background** invocation can be cut short by the lease before it exits 2; for a long wait prefer `util/safe_merge.py`, which is kill-resilient and arms a net.
...
│   ├── test_service_fork_drift.py        # Drift gate: security guards that must not diverge across the data/cascor service-core forks, canopy's APIKeyAuth and service-core itself (register §2.3; ENFORCED + self-maintaining KNOWN_GAP ledger)
...
│   ├── test_push_signed_commit.py        # Behavioural: util/push_signed_commit.py existing-branch signed commit (hermetic gh stub enforcing expectedHeadOid; full-sha pin, race refused, refusals, read-back)
...
└── util/                                 # Utility scripts and tools
    ...
    ├── push_signed_commit.py             # Cross-repo: add ONE GitHub-signed commit to an EXISTING branch, pinned to --expected-head; reads it back
...
6. **dependency-docs** -- Generates dependency documentation via the `juniper-generate-dep-docs` console script from the PyPI-published `juniper-ci-tools>=0.9.0,<0.10.0` package (replaces the legacy `util/generate_dep_docs.sh` deleted in juniper-ml#298)
...
**Known limitation (degraded no-App path only):** on the fallback path (`RELEASE_TRAIN_APP_ID` unset), a PR opened with the built-in `GITHUB_TOKEN` does **not** trigger CI workflows (GitHub's recursion guard), so a proposal PR shows **no checks** until the owner re-triggers them — close and reopen the PR, or push an empty commit. On the same token path, juniper-ml's 18 lockfile PRs (census 2026-09-22) all ran zero jobs at opening, in two shapes: 5 got no `pull_request` run when opened (#1139 got runs only after the owner pushed main into its branch), and 13 had their runs created and parked at `action_required`. For a parked PR, re-running those runs is the cheapest re-trigger, and 12 of the 13 were released that way.
...
| Opt-in | request `allow_truncation`, `JUNIPER_DATA_CSV_IMPORT_ALLOW_TRUNCATION`, or the matching `.env` entry | **Tri-state** since `APD-DATA-052`: `true` opts in, `false` REFUSES even against a deployment-wide opt-in, `null`/omitted defers to the deployment. Was a logical OR in which a client could not opt *out*. |
...
**Pre-decision-11 dataset instance (annotated 2026-09-23, juniper-ml#2034).** The counts below were scored on 2026-08-24 against the generator-1.x instances. Since decision 11 every generator is at 3.0.0 or later, and V-1 found that each one's rows changed, so a rebuild now scores a different instance and does not reproduce these counts (juniper-ml#2034). They are retained as measured.
...
**Decision 11 SHIPPED on 2026-09-06, and is ON PyPI as of 2026-09-10/11.** The NPZ contract is six keys — `X_train` / `y_train`, `X_val` / `y_val`, `X_test` / `y_test` — and the `*_full` family (including sequence `dt_full` / `target_dt_full`) is **no longer emitted by anything**. Released: juniper-data **0.14.0**, juniper-data-client **0.5.0**, juniper-cascor **0.11.0**, juniper-canopy **0.7.0**, juniper-recurrence-model **0.3.0**, juniper-recurrence-client **0.3.0**, juniper-recurrence **0.5.0**, juniper-model-core **0.3.2**. juniper-ml **0.8.0** raises its floors to the first seven of those (plus `juniper-cascor-client>=0.8.0` for the base-URL guard). **`juniper-model-core` is the exception, and the nuance matters**: `[tools]` pins it `>=0.1.0,<0.4.0`, which *admits* 0.3.2 without *requiring* it. A fresh `pip install juniper-ml[tools]` **does** get 0.3.2 — pip takes the newest satisfying version (verified 2026-09-11 against PyPI, in a clean venv). What the cap does not do is **forbid** 0.3.1: `pip install "juniper-ml[tools]==0.8.0" "juniper-model-core==0.3.1"` resolves with no conflict, so a pin, a lockfile, or a stale index can still hold a consumer on the old docstring. Left deliberately — the difference is documentation-only and a floor raise would be a consumer break for no behavioural gain. Verified from the PUBLISHED wheels rather than a checkout (`util/ad-hoc/2026-09-10_verify_published_wheels.py`): data-client 0.5.0's `NPZ_SPLITS == ("train", "val", "test")`, and recurrence-model 0.3.0's `derive_full_split` reproducing the entity-major row order for a panel artifact — which a plain concatenation does not. **A checkout is not a deployment**: model-core held the corrected `crossval/splits.py` docstring on `main` from juniper-ml#1829 while the published 0.3.1 wheel still served the retired contract, repo and PyPI both reading `0.3.1` with different contents, until 0.3.2 was cut by owner decision.
...
- **Decision 12** — `partition_provenance` inside the NPZ plus one ingestion gate. Adopted 2026-09-03 and tracked as juniper-data#423. **Specified 2026-09-22 as a v1 draft** in [`notes/JUNIPER_2026-09-22_JUNIPER-ECOSYSTEM_PARTITION-PROVENANCE-SPEC.md`](../notes/JUNIPER_2026-09-22_JUNIPER-ECOSYSTEM_PARTITION-PROVENANCE-SPEC.md). Review round 1 rated that draft unsound as written, so it is not ratifiable until a v2 folds in the findings in its §14. No code has moved.
...
| Opt-in | request `allow_truncation` (tri-state) over `JUNIPER_DATA_EQUITIES_ALLOW_TRUNCATION` / `.env` | **Tri-state** since `APD-DATA-052`: `true` opts in, `false` REFUSES even against a deployment-wide opt-in, `null`/omitted defers. Gates the symbol cap AND the unresolvable-fundamentals policy. Was a logical OR. |
...
|---|---|---:|---|
| `notes/JUNIPER_2026-03-12_JUNIPER-ML_PROMPT-ANALYSIS-AND-AUTOMATION-PLAN.md` | ` ````jinja2 ` at `:816` | **4** (`:822`, `:831`, `:840`, `:850`) | The block is a **Jinja template sample**, and those `## ` lines are the markdown the template *emits*. They are the point of the example. |
| `notes/JUNIPER_2026-08-09_JUNIPER-ECOSYSTEM_STANDING-ITEMS-CLOSEOUT-AND-HARNESS-REMEDIATION-PLAN.md` | ` ```text ` at `:1103` | **13** (`:1106`–`:1122`) | The block is **banner art**. `:1104` is a `####…####` rule and each `## ` is the banner's left border, not a heading. |

Both were re-derived against a real CommonMark parser
...
* **The jinja2 sample must not be retagged `markdown`.** Its content is a Jinja template, not
  ...
  to four backticks in [ml#1886](https://github.com/pcalnon/juniper-ml/pull/1886) precisely so
  the sample's own bytes stay untouched. Retagging trades a false positive for a false
  document.
...
| `JUNIPER_DATA_EQUITIES_ALLOW_TRUNCATION` | juniper-data | `false` | Deployment-wide opt-in to a prefix cut. Applies only when the request's tri-state `allow_truncation` is `null`/omitted; an explicit `false` refuses regardless (`APD-DATA-052`). |
...
| `JUNIPER_DATA_CSV_IMPORT_ALLOW_TRUNCATION` | juniper-data | `false`            | Deployment-wide opt-in to a partial csv_import. Applies only when the request's tri-state `allow_truncation` is `null`/omitted; an explicit `false` refuses regardless (`APD-DATA-052`). |
