...
- `util/safe_merge.py` -- Merge gate (synthesis R4): merges a PR **only** after its REQUIRED checks have finished green, refusing otherwise. Closes the measured failure where **12% of freshness-synced PRs merged before the re-test they had just paid for could finish**.
  ...
  - **CI budget is PER-REPO** (`REPO_TIMEOUTS`, `timeout_for()`), across *all required contexts on one head*, not one workflow. The prior single `900 s` was derived from ml's `ci.yml` median alone and sat at **canopy's median**, so roughly half of canopy's healthy merges would have refused with "checks did not finish".
    **Re-measured fleet-wide 2026-09-22, n=30 merged heads, required contexts only, re-run heads set aside** (`util/ad-hoc/2026-09-22_ci_budget_handoff_reprobe.py rerun-split`, over the v2 span of `util/ad-hoc/2026-09-08_measure_required_check_span_v2.py`): ml p90 910 s / max 2005; data 1651 / 2589; cascor 1615 / 2221; cascor-worker 1576 / 2059; canopy 1798 / 2174; cascor-client 1264 / 1626; data-client 1545 / 2565; deploy 450 / 965; recurrence 1114 / 1666.
    Budgets: ml/cascor 2800, cascor-worker 2400, recurrence 2000, deploy 1400, data/data-client/canopy/cascor-client 3300; `DEFAULT_TIMEOUT` 2400 remains the fallback for a repo not in the table. **Do not quote this row — re-measure.** It carried the 2026-09-08 budgets (ml 1500, recurrence 700) for thirteen days after ml#1851 raised both, and on 2026-09-22 data, data-client and deploy no longer cleared their own max and were raised. The record is `REPO_TIMEOUTS` in `util/safe_merge.py` and `MEASURED_SPANS` in `tests/test_safe_merge.py`.
    **Raw v2 counts re-run tails.** It reads check-runs with the API's default `filter=latest`, so a re-run replaces the first attempt: canopy#653 read 33,299 s for a pass that failed at 10:08 UTC and had one job re-run at 19:01. `safe_merge` reports a failure when it happens and a later invocation starts a fresh wait, so heads carrying a sequential same-name re-run are set aside before sizing.
...
- `util/wait_for_checks.py` -- Waits for a PR's **required** status checks to finish, then reports honestly. The shared replacement for the hand-rolled "wait for CI" loops that sessions keep re-writing and keep getting wrong the same two ways. Read-only (only `gh pr view` / `gh api .../rules/...` reads — never merges, updates a branch, pushes, or comments), so any session can run it at any time.
  ...
  - **Omit `--timeout` and it waits the repo's MEASURED budget** — `timeout_for` in `util/safe_merge.py` (`REPO_TIMEOUTS`), read at call time — and prints `wait budget: <s> (<source>)` on stderr; `--json` carries `timeout` and `timeout_source`. The flat 1800 s is only the fallback when that table cannot be read, and the source then says `FALLBACK`. It used to be the default outright and sat **below eight of the nine budgets**, so a direct invocation on a healthy canopy / cascor / ml PR could exit **2** while every required context was still running. `safe_merge` always passes `--timeout`, so the merge path was never affected. `TimeoutResolutionTest` pins every row.
