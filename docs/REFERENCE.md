...
- `tests/test_soak_run_probe_terminal.py` -- Hermetic gate for the BET-FAILING / HOLDS-AT- stopping rule in `util/soak_run_probe.py` (the unattended-timer spend guard). Complementary to `tests/test_soak_run_probe.py`: that suite pins retrieval-channel honesty and parse_events, and never injects a terminal verdict, so a prefix-match regression stays green. Pins that only the first `status` token is the verdict (a later `BET-FAILING` mention must not refuse), that `HOLDS-AT-*` is terminal, that `IN-PROGRESS` / `NO-DATA` / `DEGRADED` are not, that `--force` is the override, and that `--dry-run` does not bypass the guard. `util/` is outside every pre-commit Python hook, so this suite is the gate.
