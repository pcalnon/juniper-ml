...
- `tests/test_register_open_set.py` -- Hermetic gate for `util/ad-hoc/register_open_set.py`, the AUTHORITATIVE open/fixed counter the defect-register close protocol keys on (`util/` is outside every pre-commit Python hook). Pins that `**FIXED` is the only fixed token (lookalikes `FIXED` / `**CLOSED` / `**PARKED` / `*FIXED*` stay OPEN; a WON'T FIX close still writes `**FIXED` inside the marker); letter-suffix ids `001a` / `001b` are distinct; the same id in two table rows is FIXED if ANY row carries the token (union, not last-row-wins); dagger `†` rows still match; prefix grouping uses `rsplit("-", 1)` so `APD-CASCOR-001a` reports as `APD-CASCOR`; the headline counts unique ids, not raw table lines. Distinct from `register_status_crosscheck.py` (the third reading of §2 / §4 / §5.1).
...
- `util/ad-hoc/register_open_set.py` -- Re-derives the defect-register open/fixed set from §4 / §5.1 table rows. An id is FIXED if ANY of its rows carries the `**FIXED` token (the machine-readable close marker §1 cites). Inventing a second marker leaves the row counted OPEN. CWD-relative `notes/JUNIPER_2026-08-14_JUNIPER-ECOSYSTEM_DEFECT-REGISTER.md`. Distinct from `register_status_crosscheck.py`, which compares three independent readings. Tests: `tests/test_register_open_set.py`.
