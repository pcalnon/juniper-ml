...
**Decision 11 shipped 2026-09-06 and is ON PyPI 2026-09-10/11:** the `*_full` family is gone from the contract. Released as juniper-data 0.14.0 / data-client 0.5.0 / cascor 0.11.0 / canopy 0.7.0 / recurrence-model 0.3.0 / recurrence-client 0.3.0 / recurrence 0.5.0 / model-core 0.3.2. juniper-ml 0.8.0 floors the first seven (and cascor-client at `>=0.8.0` for the base-URL guard); **model-core is *admitted*, not required**, by `[tools]`' `>=0.1.0,<0.4.0`: a fresh install DOES get 0.3.2 (pip takes newest satisfying), but the cap does not forbid 0.3.1, so a pin or lockfile can still hold a consumer on the old docstring — a docstring-only difference. Two rules — **never require `X_full`** (nothing emits it), and **never assert it is absent** (every artifact stored before 2026-09-06 has it, and consumers must keep loading those). Recurrence YAML `dataset.split` accepts `{train, val, test, full}`; the long form `"validation"` is exit 2.
