...
Tip: `e2e_seg17_topology_driver.py` scores Topology-tab rows against isolated `:8051`. Trust the `STEPS` dict (`--step` exit `2` on unknown names) — the docstring still lists M-TOPOLOGY-13/-14 as unimplemented, but `topostate` / `topoexport` exist. On `main`, M-06 is `label == want OR hidden count`; M-07 asserts container display only; M-12 FAIL-scores empty-space clear. Full contract: [REFERENCE — Canopy E2E Topology Driver](REFERENCE.md#canopy-e2e-topology-driver).
