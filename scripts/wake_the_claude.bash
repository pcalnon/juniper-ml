...
function save_session_id() {
    ...
    debug_log "Saved Session ID $(redact_uuid "${session_id}") to file: ${safe_filename}"
}

# Retrieve Session ID from file
function retrieve_session_id() {
    local session_id_filename="$1"
    debug_log "Retrieving Session ID from file: ${session_id_filename}" >&2
    local session_id
...
function maybe_remove_generated_session_id_file() {
    ...
    if [[ "${session_id_filename}" == "${expected_filename}" ]]; then
        echo "Removing generated session id file: \"${session_id_filename}\"" >&2
...
function validate_session_id() {
    ...
    if [[ "${session_id}" == "" ]]; then
        ...
        debug_log "Session ID is a file: \"${session_id_filename}\"" >&2
...
if [[ -f "nohup.out" ]]; then
    debug_log "Removing existing nohup.out"
    rm -f "nohup.out"
fi

debug_log "nohup claude [${#CLAUDE_CODE_PARAMS[@]} args] &"
