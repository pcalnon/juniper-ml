...
########################################################################################################################################################################
debug_log "wake_the_claude: Define Claude Code parameter flags"
...
debug_log "wake_the_claude: Define Claude Code Effort values"
EFFORT_LOW="low"
EFFORT_MED="medium"
EFFORT_HIGH="high"
...
########################################################################################################################################################################
debug_log "wake_the_claude: Define Input parameter flags"
...
########################################################################################################################################################################
debug_log "wake_the_claude: Define Test Input parameters"
...
debug_log "wake_the_claude: Default Testing Input parameters: \"${PARAMS_TEST}\""
...
function is_valid_uuid() {
    ...
    local uuid="$1"
    debug_log "wake_the_claude: Validating UUID format" >&2
    if [[ ${uuid//-/} =~ ^[[:xdigit:]]{32}$ ]]; then
        debug_log "wake_the_claude: UUID is valid" >&2
        return 0
    else
        debug_log "wake_the_claude: UUID is invalid" >&2
        return 1
    fi
}
...
function generate_uuid() {
    ...
    if command -v uuidgen >/dev/null 2>&1; then
        ...
        if is_valid_uuid "${generated_uuid}"; then
            debug_log "wake_the_claude: Generated UUID: ${generated_uuid}"
            return "${TRUE}"
        fi
    fi
...
function save_session_id() {
    ...
    debug_log "wake_the_claude: Extracting Session ID and saving to file"
    local session_id
    session_id="$(echo "${session_id_value}" | awk -F " " '{print $2;}')"
    if ! is_valid_uuid "${session_id}"; then
        debug_log "wake_the_claude: Error: Session ID is not a valid UUID — refusing to write file" >&2
        return "${FALSE}"
    fi
    local safe_filename
    ...
    if [[ -L "${target_path}" ]]; then
        debug_log "wake_the_claude: Error: target file is a symlink — refusing to write" >&2
        return "${FALSE}"
    fi
    debug_log "wake_the_claude: Saving Session ID: ${session_id} to file: ${target_path}"
    debug_log "wake_the_claude: Saved Session ID $(redact_uuid "${session_id}") to file: ${target_path}"
}

# Retrieve Session ID from file
function retrieve_session_id() {
    local session_id_filename="$1"
    debug_log "wake_the_claude: Retrieving Session ID from file: ${SESSIONS_DIR}/${session_id_filename}" >&2
    local session_id
    session_id="$(cat "${SESSIONS_DIR}/${session_id_filename}")"
    debug_log "wake_the_claude: Completed retrieving Session ID from file: \"${session_id_filename}\"" >&2
    debug_log "wake_the_claude: Retrieved Session ID: ${session_id}"
}
...
function maybe_remove_generated_session_id_file() {
    ...
    if [[ "${session_id_filename}" == "${expected_filename}" ]]; then
        debug_log "wake_the_claude: Removing generated session id file: \"${SESSIONS_DIR}/${session_id_filename}\"" >&2
        rm -f "${SESSIONS_DIR}/${session_id_filename}"
        debug_log "wake_the_claude: Completed removing generated session id file: \"${session_id_filename}\"" >&2
    else
        debug_log "wake_the_claude: Preserving session id file because filename is not generated by this script: \"${session_id_filename}\"" >&2
    fi
}
...
function validate_session_id() {
    ...
    debug_log "wake_the_claude: Validating Session ID: $(redact_uuid "${session_id}")" >&2
    if [[ "${session_id}" == "" ]]; then
        debug_log "wake_the_claude: Session ID is empty" >&2
        return "${FALSE}"
    elif is_valid_uuid "${session_id}"; then
        debug_log "wake_the_claude: Session ID is valid: $(redact_uuid "${session_id}")" >&2
    elif [[ "${session_id}" == */* ]]; then
        debug_log "wake_the_claude: Session ID filename contains path separators — rejected" >&2
        return "${FALSE}"
    elif [[ "${session_id}" != *.txt ]]; then
        debug_log "wake_the_claude: Session ID filename must have .txt extension — rejected" >&2
        return "${FALSE}"
    elif [[ -f "${SESSIONS_DIR}/${session_id}" ]]; then
        session_id_filename="${session_id}"
        debug_log "wake_the_claude: Session ID is a file: \"${SESSIONS_DIR}/${session_id_filename}\"" >&2
        session_id="$(retrieve_session_id "${session_id_filename}")"
        debug_log "wake_the_claude: Completed retrieving Session ID from file: \"${session_id_filename}\"" >&2
        if is_valid_uuid "${session_id}"; then
            debug_log "wake_the_claude: Session ID is valid: $(redact_uuid "${session_id}")" >&2
        else
            debug_log "wake_the_claude: Session ID file did not contain a valid UUID" >&2
            return "${FALSE}"
        fi
    else
        debug_log "wake_the_claude: Session ID is invalid" >&2
        return "${FALSE}"
    fi
    echo "${session_id}"
}
...
function is_valid_uuid() {
    # example uuid="7632f5ab-4bac-11e6-bcb7-0cc47a6c4dbd"
    local uuid="$1"
    debug_log "wake_the_claude: Validating UUID: \"${uuid}\""
    if [[ ${uuid//-/} =~ ^[[:xdigit:]]{32}$ ]]; then
        debug_log "wake_the_claude: UUID is valid: \"${uuid}\""
        return 0
    else
        debug_log "wake_the_claude: UUID is invalid: \"${uuid}\""
        return 1
    fi
}
...
function save_session_id() {
    local session_id_value="$1"
    debug_log "wake_the_claude: Extract Session ID from Session ID Value: \"${session_id_value}\" and save to file"
    session_id="$(echo "${session_id_value}" | awk -F " " '{print $2;}')"
    echo "${session_id}" > "${session_id}.txt"
    debug_log "wake_the_claude: Completed extracting Session ID: \"${session_id}\" from Session ID Value: \"${session_id_value}\""
}

# Retrieve Session ID from file
function retrieve_session_id() {
    local session_id_filename="$1"
    debug_log "wake_the_claude: Retrieve saved Session ID from file: ${session_id_filename}"
    session_id="$(cat "${session_id_filename}")"
    debug_log "wake_the_claude: Completed retrieving saved Session ID: \"${session_id}\" from file: \"${session_id_filename}\""
    debug_log "wake_the_claude: Removing file: \"${session_id_filename}\""
    rm -f "${session_id_filename}"
    debug_log "wake_the_claude: Completed removing file: \"${session_id_filename}\""
    echo "${session_id}"
}

function validate_session_id() {
    local session_id="$1"
    debug_log "wake_the_claude: Validating Session ID: \"${session_id}\""
    if [[ "${session_id}" == "" ]]; then
        debug_log "wake_the_claude: Session ID is empty: \"${session_id}\""
        debug_log "wake_the_claude: Error: Session ID is empty. Exiting..." >&2
        usage "${FALSE}"
    elif is_valid_uuid "${session_id}"; then
        debug_log "wake_the_claude: Session ID is valid: \"${session_id}\""
    elif [[ -f "./${session_id}" ]]; then
        debug_log "wake_the_claude: Session ID is a file: \"${session_id}\""
        session_id="$(retrieve_session_id "${session_id}")"
        debug_log "wake_the_claude: Completed retrieving Session ID: \"${session_id}\" from file: \"${session_id}\""
        if is_valid_uuid "${session_id}"; then
            debug_log "wake_the_claude: Session ID is valid: \"${session_id}\""
        else
            debug_log "wake_the_claude: Session ID is invalid: \"${session_id}\""
            debug_log "wake_the_claude: Error: Session ID is invalid. Exiting..."
            usage "${FALSE}"
        fi
    else
        debug_log "wake_the_claude: Session ID is invalid: \"${session_id}\""
    ...
    debug_log "wake_the_claude: Completed validating Session ID: \"${session_id}\""
}
...
########################################################################################################################################################################
debug_log "wake_the_claude: Define Valid Prompt Boolean Flags"
...
# Parsed Param values
########################################################################################################################################################################
debug_log "wake_the_claude: Define Parsed Param values"
...
########################################################################################################################################################################
debug_log "wake_the_claude: Define Param values to be passed to Claude"
...
########################################################################################################################################################################
debug_log "wake_the_claude: Verify that input parameters have been provided"
if [[ "${*}" != "" ]]; then
    debug_log "wake_the_claude: Input Params: [${#} args]"
else
    debug_log "wake_the_claude: No input params provided."
    debug_log "wake_the_claude: Next time try these:"
    debug_log "wake_the_claude: ${PARAMS_TEST}"
    usage "${FALSE}"
fi
debug_log "wake_the_claude: Completed verifying that input parameters have been provided"

debug_log "wake_the_claude: Parse input parameters"
while [[ "${TRUE}" != "${FALSE}" ]]; do

    if [[ "${1}" != "" ]]; then
        CURRENT_ELEMENT="${1}"
        shift
        debug_log "wake_the_claude: Current Flag: \"${CURRENT_ELEMENT}\""
    else
        debug_log "wake_the_claude: Completed parsing input params"
        break
    fi
    if matches_pattern "${CURRENT_ELEMENT}" "${PATH_FLAGS}"; then
        debug_log "wake_the_claude: Parsing path flags"
        if [[ ( "${1}" != "" ) && ( "${1:0:2}" != "${SPACER_FLAGS}" ) ]]; then
            PATH_NAME="${1}"
            shift
        else
            debug_log "wake_the_claude: Error: Received Path Flag but no Path Value. Exiting..."
            usage "${FALSE}"
        fi
        debug_log "wake_the_claude: Completed Parsing path value"
        if [[ "${PATH_NAME: -1}" == "/" ]]; then
            PATH_NAME="${PATH_NAME%\/*}"
            debug_log "wake_the_claude: Removed trailing slash from path"
        fi
        if [[ -f "${PATH_NAME}" ]]; then
            VALID_PATH_PARAM="${TRUE}"
            PROMPT_FILE="${PATH_NAME}"
            debug_log "wake_the_claude: Provided Pathname is a valid file"
        elif [[ -d "${PATH_NAME}" ]]; then
            debug_log "wake_the_claude: Provided Pathname is a valid directory"
            if [[ ( "${FILE_NAME}" != "" ) && ( -f "${PATH_NAME}/${FILE_NAME}" ) ]]; then
                VALID_PATH_PARAM="${TRUE}"
                PROMPT_FILE="${PATH_NAME}/${FILE_NAME}"
                debug_log "wake_the_claude: Combined Pathname and Filename is a valid file"
            elif [[ "${FILE_NAME}" == "" ]]; then
                debug_log "wake_the_claude: Filename has not yet been parsed"
                continue
            else
                debug_log "wake_the_claude: Error: Prompt file is invalid when Pathname is combined with Filename. Exiting..."
                usage "${FALSE}"
            fi
        else
            debug_log "wake_the_claude: Error: received an invalid Prompt File Path. Exiting..."
            usage "${FALSE}"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${FILE_FLAGS}"; then
        debug_log "wake_the_claude: Parsing file flags"
        if [[ ( "${1}" != "" ) && ( "${1:0:2}" != "${SPACER_FLAGS}" ) ]]; then
            FILE_NAME="${1}"
            shift
        else
            debug_log "wake_the_claude: Error: Received File Flag but no File Value. Exiting..."
            usage "${FALSE}"
        fi
        ...
        if [[ -f "${FILE_NAME}" ]]; then
            ...
            debug_log "wake_the_claude: Provided Filename is a valid file"
        elif [[ ( "${PATH_NAME}" != "" ) && ( -f "${PATH_NAME}/${FILE_NAME}" ) ]]; then
            VALID_FILE_PARAM="${TRUE}"
            PROMPT_FILE="${PATH_NAME}/${FILE_NAME}"
            debug_log "wake_the_claude: Combined Pathname and Filename is a valid file"
        elif [[ ${PATH_NAME} == "" ]]; then
            debug_log "wake_the_claude: Filename not yet valid, Pathname has not yet been parsed"
            continue
        else
            debug_log "wake_the_claude: Error: received an invalid Prompt File. Exiting..."
            usage "${FALSE}"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${RESUME_FLAGS}"; then
        debug_log "wake_the_claude: Parsing resume flags"
        if [[ ( "${1}" != "" ) && ( "${1:0:2}" != "${SPACER_FLAGS}" ) ]]; then
            SESSION_ID="${1}"
            shift
            debug_log "wake_the_claude: Validating Session ID"
            SESSION_ID="$(validate_session_id "${SESSION_ID}")"
            RETURN_VALUE=$?
            if [[ ( "${RETURN_VALUE}" == "${TRUE}" ) && ( "${SESSION_ID}" != "" ) ]]; then
                debug_log "wake_the_claude: Session ID validated: $(redact_uuid "${SESSION_ID}")"
                CLAUDE_CODE_PARAMS+=("${CLAUDE_RESUME_FLAGS}" "${SESSION_ID}")
                debug_log "wake_the_claude: Completed parsing resume, ${#CLAUDE_CODE_PARAMS[@]} args"
            else
                debug_log "wake_the_claude: Error: Session ID is invalid. Exiting..."
                usage "${FALSE}"
            fi
        else
            debug_log "wake_the_claude: Error: Received Resume Flag but no Valid Session ID to Resume. Exiting..."
            usage "${FALSE}"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${SESSION_ID_FLAGS}"; then
        debug_log "wake_the_claude: Parsing session id flags"
        if [[ ( "${1}" != "" ) && ( "${1:0:2}" != "${SPACER_FLAGS}" ) ]]; then
            SESSION_ID_VALUE="${CLAUDE_SESSION_ID_FLAGS} ${1}"
            CLAUDE_CODE_PARAMS+=("${CLAUDE_SESSION_ID_FLAGS}" "${1}")
            shift
            debug_log "wake_the_claude: Received Session ID, ${#CLAUDE_CODE_PARAMS[@]} args"
        else
            debug_log "wake_the_claude: Warning: Received Session ID Flag but no Session ID Name."
            debug_log "wake_the_claude: Session ID Value not Provided, Assigning a new UUID as Session ID."
            generated_uuid="$(generate_uuid)"
            if [[ ( "$?" != "${TRUE}" ) || ( "${generated_uuid}" == "" ) ]]; then
                debug_log "wake_the_claude: Error: Failed to generate a valid UUID for Session ID."
                usage "${FALSE}"
            fi
            SESSION_ID_VALUE="${CLAUDE_SESSION_ID_FLAGS} ${generated_uuid}"
            CLAUDE_CODE_PARAMS+=("${CLAUDE_SESSION_ID_FLAGS}" "${generated_uuid}")
            debug_log "wake_the_claude: Generated new Session ID: $(redact_uuid "${generated_uuid}"), ${#CLAUDE_CODE_PARAMS[@]} args"
        fi
        if ! save_session_id "${SESSION_ID_VALUE}"; then
            debug_log "wake_the_claude: Error: Session ID value is invalid. Exiting..."
            usage "${FALSE}"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${WORKTREE_FLAGS}"; then
        debug_log "wake_the_claude: Parsing worktree flags"
        if [[ ( "${1}" != "" ) && ( "${1:0:2}" != "${SPACER_FLAGS}" ) ]]; then
            # WORKTREE_VALUE="${CLAUDE_WORKTREE_FLAGS} ${1}"
            CLAUDE_CODE_PARAMS+=("${CLAUDE_WORKTREE_FLAGS}" "${1}")
            shift
            debug_log "wake_the_claude: Received Worktree Value, ${#CLAUDE_CODE_PARAMS[@]} args"
        else
            debug_log "wake_the_claude: Warning: Received Worktree Flag but no Worktree Name. Letting Claude Code assign one."
            WORKTREE_VALUE="${CLAUDE_WORKTREE_FLAGS}"
            CLAUDE_CODE_PARAMS+=("${WORKTREE_VALUE}")
            debug_log "wake_the_claude: Received Worktree Flag (no value), ${#CLAUDE_CODE_PARAMS[@]} args"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${EFFORT_FLAGS}"; then
        debug_log "wake_the_claude: Parsing effort flags"
        if [[ ( "${1}" != "" ) && ( "${1:0:2}" != "${SPACER_FLAGS}" ) && ( ( "${1}" == "${EFFORT_LOW}" ) || ( "${1}" == "${EFFORT_MED}" ) || ( "${1}" == "${EFFORT_HIGH}" ) ) ]]; then
            ...
            shift
            debug_log "wake_the_claude: Received Effort Value, ${#CLAUDE_CODE_PARAMS[@]} args"
        else
            debug_log "wake_the_claude: Error: Received Effort Flag but no valid Effort Value. Exiting..."
            usage "${FALSE}"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${MODEL_FLAGS}"; then
        debug_log "wake_the_claude: Parsing model flags"
        if [[ "${1}" != "" ]]; then
            # TODO: Validate Model value
            # MODEL_VALUE="${CLAUDE_MODEL_FLAGS} ${1}"
            CLAUDE_CODE_PARAMS+=("${CLAUDE_MODEL_FLAGS}" "${1}")
            shift
            debug_log "wake_the_claude: Received Model Value, ${#CLAUDE_CODE_PARAMS[@]} args"
        else
            debug_log "wake_the_claude: Error: Received Model Flag but no Model Name. Exiting..."
            usage "${FALSE}"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${PROMPT_FLAGS}"; then
        debug_log "wake_the_claude: Parsing prompt flags"
        if [[ "${1}" != "" ]]; then
            PROMPT_VALUE="${1}"
            shift
            VALID_PROMPT_PARAM="${TRUE}"
            debug_log "wake_the_claude: Received prompt [${#PROMPT_VALUE} chars]"
        else
            debug_log "wake_the_claude: Error: Did not receive a valid prompt string. Exiting..."
            usage "${FALSE}"
        fi
    elif matches_pattern "${CURRENT_ELEMENT}" "${VERSION_FLAGS}"; then
        debug_log "wake_the_claude: Parsing version flags"
        claude "${CLAUDE_VERSION_FLAGS}"
        exit "${TRUE}"
    elif matches_pattern "${CURRENT_ELEMENT}" "${HEADLESS_FLAGS}"; then
        debug_log "wake_the_claude: Parsing headless flags"
        HEADLESS_VALUE="${CLAUDE_HEADLESS_FLAGS}"
        CLAUDE_CODE_PARAMS+=("${HEADLESS_VALUE}")
        debug_log "wake_the_claude: Received Headless Flag, ${#CLAUDE_CODE_PARAMS[@]} args"
    elif matches_pattern "${CURRENT_ELEMENT}" "${PERMISSIONS_FLAGS}"; then
        debug_log "wake_the_claude: Parsing permissions flags"
        PERMISSIONS_VALUE="${CLAUDE_PERMISSIONS_FLAGS}"
        CLAUDE_CODE_PARAMS+=("${PERMISSIONS_VALUE}")
        debug_log "wake_the_claude: Received Permissions Flag, ${#CLAUDE_CODE_PARAMS[@]} args"
    elif matches_pattern "${CURRENT_ELEMENT}" "${SPACER_FLAGS}"; then
        debug_log "wake_the_claude: Received Spacer Flag"
        continue
        ...
        debug_log "wake_the_claude: Error: Received Invalid Input Param: \"${CURRENT_ELEMENT}\""
        usage "${FALSE}"
    fi
    debug_log "wake_the_claude: Completed Parsing: \"${CURRENT_ELEMENT}\", ${#CLAUDE_CODE_PARAMS[@]} args"
done
debug_log "wake_the_claude: Completed Parsing input parameters"


########################################################################################################################################################################
# Build Claude Code Prompt
########################################################################################################################################################################
debug_log "wake_the_claude: Build Claude Code Prompt"
...
if [[ "${CLAUDE_CODE_PROMPT}" != "" ]]; then
    ...
    debug_log "wake_the_claude: Prompt loaded [${#CLAUDE_CODE_PROMPT} chars]"
fi
debug_log "wake_the_claude: Completed building prompt, ${#CLAUDE_CODE_PARAMS[@]} total args"


########################################################################################################################################################################
# Execute Claude Code
########################################################################################################################################################################
debug_log "wake_the_claude: Executing claude with ${#CLAUDE_CODE_PARAMS[@]} args"

CLAUDE_BIN="$(type -P claude 2>/dev/null || true)"
if [[ "${CLAUDE_BIN}" == "" ]] || [[ ! -x "${CLAUDE_BIN}" ]]; then
    debug_log "wake_the_claude: Error: claude command not found in PATH" >&2
    exit 1
fi
...
if [[ "${HEADLESS_VALUE}" != "" ]]; then
    if [[ "${NOHUP_LOG_FILE}" != "" ]]; then
        ...
        debug_log "wake_the_claude: nohup claude ${CLAUDE_CODE_PARAMS[*]} &"
        nohup "${CLAUDE_BIN}" "${CLAUDE_CODE_PARAMS[@]}" &
    fi
else
    debug_log "wake_the_claude: \"${CLAUDE_BIN}\" ${CLAUDE_CODE_PARAMS[*]}"
    "${CLAUDE_BIN}" "${CLAUDE_CODE_PARAMS[@]}"
fi
NOHUP_STATUS=$?
if [[ "${NOHUP_STATUS}" != "0" ]]; then
    debug_log "wake_the_claude: Error: Failed to launch claude with nohup" >&2
    exit 1
fi
debug_log "wake_the_claude: Completed Executing Claude Code"
exit 0
