...
**This file's own consensus validation** is archived as `reports/2026-09-24_defect-register-round-42/handoff-consolidated-round*-*.md`, and the frozen copies its reports (and document 1's) cite by line are in `handoff-frozen/` beside them, with a `README.md` that maps each (document 2 at `2e4917c2` and document 3 on `main` are not copied: git holds them). Both ship in the consolidation PR, on branch `docs/handoff-round42-consolidated`. It ran four rounds: lanes F (re-probe), O (amputation hunt) and P (fresh-session walk) in rounds 1 and 2; F and P in round 3; F and O in round 4, a delta check on round 3's corrections and on the text moved out of the Goal. Every lane returned PASS WITH CORRECTIONS. Round 1 had two HIGH findings, and rounds 2-4 none. The amputation hunts of rounds 1 and 2 found details dropped from the sources (round 1, two lost items of document 1), all restored; round 4's found nothing lost. Every correction is applied; round 4's, all LOW, NIT or a MEDIUM with the lane's own replacement text, were applied without a fifth round.

**Length:** the Goal is about 1,480 words (1,477 by `wc -w`, from its heading to "Key context"). `~/.claude/CLAUDE.md` says "Keep it under ~1,200 words", a target near the median; the canonical procedure (`notes/JUNIPER_2026-02-23_JUNIPER-ML_THREAD-HANDOFF-PROCEDURE.md`, "On the ~1,200 figure") treats the IQR, 778–2,718, as the real guidance. This file merges three handoffs and carries two lanes' first actions and live state; what could move to an appendix without loss has moved.
...
2. **Is "defect reg [24f8d8]" still live?** Run `ListAgents`, loading it with ToolSearch if it is deferred; if it is unavailable, ask the owner whether that session is open. `ListAgents`' header line names your own session and `[ref]`; [24f8d8] appears among the peer sessions in any state but offline. Its executor finished at 01:49Z (In flight).
   - **Listed:** message it first, with your name and `[ref]`. Say you are its successor (the lane split is settled at First action 3). Ask (1) whether it has opened, or will open, the consolidation PR, and when; (2) whether any of its subagents is still running; (3) whether it still writes in its worktree. If it will open the PR, let it (ship none of its files; carry on with First actions 3 and 4): send the first message with `notify_when_idle: true` (a one-shot notice); if [24f8d8] goes idle or exits with no PR, ask once more, again with `notify_when_idle: true`, and treat it as silent if that notice also comes with no PR. Otherwise ask it to launch and ship nothing more. Ask it to forward anything a Lane F successor sends it (document 2's Step 0 messages [24f8d8] first). Start no item it says it is doing.
...
- juniper-ml#2089 (`86b497cb`, since the handoff-validation probes were added at 02:39Z, 03:03Z and 03:38Z) and #2097 (`2e4917c2`), CodeQL failing on both, neither armed (BEHIND at writing, because `main` moved);
...
- **Where fixes go.** An OPEN PR gets a signed fixup. A MERGED PR gets a new PR from fresh `main` (`util/open_signed_pr.py`). Read each PR's state with `gh pr view <N> --json state,headRefOid,mergeCommit` at push time. Validate every FIX, whether a fixup or a fix-forward PR, and every other PR that is not evidence-only (the fork-drift marker; a cap, lock or floor PR), of either lane, with at least two lanes (document 2, OPEN 4); a row closes only once its fix-forward's validation also holds (Appendix A, Gates). Evidence-only commits (archived reports, `MISSING` entries, probes, handoffs) need only the archiver's `--check` and CI.
...
python3 util/ad-hoc/2026-09-24_archive_round42_reports.py --check | grep -c 'OK    '   # positive (55 in fizzy at 03:40Z; 34 in a worktree at origin/main until the consolidation PR merges); 0 means it exited early (a cited transcript is gone)
...
ls /home/pcalnon/Development/python/Juniper/juniper-ml/.claude/worktrees/fizzy-hugging-dream/reports/2026-09-24_defect-register-round-42/ | grep -c -e data438-fixforward -e handoff-2fba4397 -e handoff-consolidated -e owner-ruling-key-leaks   # 23 at 03:40Z, after round 4; +1 per report archived since (this file's later rounds, and Work 1's `data438-fixforward-round2-*`): Lane R's reports that ship in the consolidation PR
...
- **Lane R, worktree `fizzy-hugging-dream`** (branch `worktree-fizzy-hugging-dream`).
  ...
  - Its uncommitted files ship in the consolidation PR. They are:
    ...
    - `util/ad-hoc/2026-09-24_rebuild_round42_doc2_draft_0021z.py`, which rebuilt `handoff-frozen/doc2-draft-0021z.md` from session `bc31e993`'s transcript;
  ...
  - **Their probes, too.** Every validation lane leaves its probe scripts in `/tmp/claude-1000/-home-pcalnon-Development-python-Juniper-juniper-ml/2fba4397-7d9b-4929-8ca2-375b8168e1c8/scratchpad/hc<k>/lane<X>/` (tmpfs only). Rounds 1-4's surviving probes are on #2089 (Appendix B); round 1's lane P deleted its own, and the O lanes of rounds 1-3 wrote none. For any later round with no `handoff-consolidated-round<k>-lane<X>/` directory in #2089's file list (`gh api --paginate repos/pcalnon/juniper-ml/pulls/2089/files --jq '.[].filename'`), in this order:
    1. Check that fizzy's `util/ad-hoc/2026-09-24_round42_probes/README.md`, copier (`util/ad-hoc/2026-09-24_copy_round42_session2fba4397_probe_scripts.py`) and pusher (`util/ad-hoc/2026-09-24_push_round42_handoff_probes_to_2089.py`) equal #2089's head (`gh api 'repos/pcalnon/juniper-ml/contents/<path>?ref=<head>' --jq .content | base64 -d | diff - <fizzy path>`), and rebuild any that differ from that head.
    2. Add `"hc<k>/lane<X>": "handoff-consolidated-round<k>-lane<X>"` to the copier's `LANES` and the directory to the pusher's `DIRS` (only for lanes whose scratch directory holds `.py` files), and add a README row.
    3. Run the copier by absolute path, then again with `--lint` (pre-commit over every preserved probe).
    4. Run `python3 /home/pcalnon/Development/python/Juniper/juniper-ml/.claude/worktrees/fizzy-hugging-dream/util/ad-hoc/2026-09-24_push_round42_handoff_probes_to_2089.py --expected-head <#2089's full headRefOid, from gh> --message '<headline>' --commit-body-file <ABSOLUTE path> --dry-run`, then again without `--dry-run`. The commit body ends with the attribution trailers as its last paragraph.
...
- **[R] The next juniper-data release.** 0.16.0 ships the defects #438 fixed. The next release carries #438, the fix-forward, data#440 and X8. Ask: cut it once the data fix-forward merges and its validation holds? (Yes or no; the owner cuts it.)
...
- **`d1c66a11`** (the executor's first push, at 20:45Z) answers every finding in `data438-round1-lane{A-reprobe,B-refute}.md`, two only partly (lane B's M2(b) and L2). data438's F8 is left to the owner; L5 and L7 are disclosed. Its numbers: unit 1901, coverage 97.76%, api+integration 118, harness "PASS: 70 mutations", equivalence 3,495,583 / 0.
...
- **Lane R** (session `2fba4397`):
  ...
  - **On #2089 (244 files at `86b497cb`):** `util/ad-hoc/2026-09-24_round42_probes/README.md` and sixteen lane directories (`register-fixforward2-round{1,2}-lane{A,B}`, `data438-fixforward-round1-lane{A,B}`, and the ten handoff-validation directories), `util/ad-hoc/2026-09-24_copy_round42_session2fba4397_probe_scripts.py`, `util/ad-hoc/2026-09-24_open_round42_session2fba4397_probes_pr.py` and `util/ad-hoc/2026-09-24_push_round42_handoff_probes_to_2089.py`.
  - **In the consolidation PR:** the files listed under Git status, including this file. Among them is `reports/2026-09-24_defect-register-round-42/handoff-frozen/`: the frozen handoff copies the validation reports cite by line ("L<n>"), one per round, which were on tmpfs; document 2's 00:21Z draft, rebuilt from session `bc31e993`'s transcript by `util/ad-hoc/2026-09-24_rebuild_round42_doc2_draft_0021z.py` (which also ships); and a `README.md` mapping each report's scratch path and sha256 to its file. Two cited documents are not copied, because git holds the same bytes: document 2 at `2e4917c2`, and document 3 on `main`.
...
- **Document 3 (the predecessor, stale),** as document 1's Appendix D mapped it:
  ...
  | Git status (worktree `hazy-beaming-map`) | Nothing unshipped: its scripts are on `main`, two at earlier versions, and two of its 203 probe files are pre-redaction copies #2081 replaced; a cleanup candidate (Appendix F's gate). |
...
- **Document 1 (Lane R):**
  ...
  | In flight | In flight (finished), Appendix C, and Appendix J (Work 1's brief records and corrections) |
...
- **Document 2 (Lane F):**
  ...
  | Step 0 | First actions 2-4, and the owner's policy |
  | Goal and split | Goal |
...
**Work 5: #2088's one unvalidated delta**, `990ef3f9..2439d049` (round 2's corrections). Its branch was deleted on merge; both commits stay reachable through `refs/pull/2088/head` (`1f116c38`, which is `2439d049` plus a `main` merge). Fetch it first, `git fetch origin refs/pull/2088/head`: a worktree whose stale remote-tracking ref happens to keep the objects is not proof that yours has them. Two lanes over the whole range; nothing waits on it and it waits on nothing. The checklist:
