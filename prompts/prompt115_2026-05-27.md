...
The following juniper project applications are involved in a regressions or have experienced significant regressions:
...
The containerized, docker compose managed, juniper applications stack is currently failing.

- canopy training/operational status is error
- ws fails to connect, appears to be in reconnection loop
- regressions from Dataset View tab:
  - unable to save dataset related changes to meta-parameters
  - unable to generate a new dataset based on the new meta-parameters
  - unable to launch cascor network training from canopy

## Objectives

address the regressions by:

- researching and analyzing these issues
- investigating the relevant source code sections
- validating these issues
- developing fixes for these issues
- validate the fixes
- implement the fixes
- add additional tests as needed to ensure fixes work as intended
- add additional tests as needed to ensure no additional regressions have been introduced
- ensure original issues have actually been resolved
