...
| R1.3  | seed-04 worker heartbeat     | **main work done** | Paul Calnon | juniper-cascor#150, juniper-cascor-worker#37, juniper-deploy#43 | cleaned | All 3 main PRs merged 2026-04-28. Chart 1.0.0→1.1.0 with `worker.healthcheck.enabled=false` default; worker HTTP listener + enriched heartbeat live; cascor accepts new fields. |
| R1.3.4 | flag-flip follow-up        | deferred (burn-in) |             |                                       |          | Bump `worker.healthcheck.enabled` default to `true`, chart 1.1.0→1.2.0, after staging confirms worker image ≥ 0.4.0 stable in production-shaped traffic. |
| R2.1  | seed-06 shared lib           | design pending |             |                                       |          | Awaiting [METRICS_MONITORING_R2.1_SHARED_OBSERVABILITY_DESIGN_2026-04-28.md](METRICS_MONITORING_R2.1_SHARED_OBSERVABILITY_DESIGN_2026-04-28.md). 5-PR sequence ending with `juniper-observability==0.1.0` on PyPI. |
