...
| R3.4   | seed-09 Sentry unconditional | **done**           | Paul Calnon | (audit-closure, no code PR)                   | n/a      | Audit found zero `importorskip("sentry_sdk")` in any repo — seed-09 premise stale. Hook tests already run unconditionally via `with patch("sentry_sdk.init")` + `monkeypatch.setitem(sys.modules, "sentry_sdk", _FakeSentry)`. See [`METRICS_MONITORING_R3.4_SENTRY_AUDIT_CLOSURE_2026-04-30.md`](METRICS_MONITORING_R3.4_SENTRY_AUDIT_CLOSURE_2026-04-30.md). |
