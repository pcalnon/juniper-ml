...
| Stale cascor image in running stack — new `JUNIPER_CASCOR_WS_CONTROL_ALLOWED_ORIGINS` env var ignored: running image before Apr A merged | Medium     | Critical (looks like the fix didn't take)   | Add image-build-time `BUILD_INFO_IMAGE_CREATED` inspection to §4 accept script. Reference memory `project_juniper_cascor_image_2_months_stale_2026-05-27`. |
...
| Origin-related test failures on the existing cascor pytest suite from Settings field-mode change                                         | Low        | Medium                                      | Change additive: (adds env alias; keeps default list). Tests use programmatic `Settings(ws_control_allowed_origins=[...])` - unaffected by env aliasing.   |
