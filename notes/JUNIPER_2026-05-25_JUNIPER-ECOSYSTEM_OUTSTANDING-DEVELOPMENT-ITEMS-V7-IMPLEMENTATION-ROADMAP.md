...
> **Audit-driven**: see [`JUNIPER_2026-05-05_JUNIPER-ECOSYSTEM_ROADMAP-AUDIT.md`](./JUNIPER_2026-05-05_JUNIPER-ECOSYSTEM_ROADMAP-AUDIT.md) for the parallel five-agent audit + validation pass that surfaced these deltas.
...
A separate **async-route audit hook migration plan** ([juniper-ml #222](https://github.com/pcalnon/juniper-ml/pull/222), to land at `notes/JUNIPER_2026-05-07_JUNIPER-ECOSYSTEM_ASYNC-ROUTE-AUDIT-HOOK-MIGRATION-PLAN.md`) addresses the BUG-JD-10 bug class going forward.
...
| **CFG-01** | §20 | ✅ shipped 2026-05-23 | canopy [PR #315](https://github.com/pcalnon/juniper-canopy/pull/315) (`9fa1f46 chore(deps): CFG-01 — add [demo] optional-dependencies extra with torch>=2.0.0`); torch declared in a new `[demo]` extra rather than core deps per roadmap §13452 recommendation (keeps the ~2 GB install footprint opt-in for production deployments driving a remote cascor service via `[juniper-cascor]`). `[dev]` aggregator updated to include `[demo]` so the test suite (which imports torch unconditionally) resolves under `pip install juniper-canopy[dev]` |
| **CFG-02** | §20 | ✅ shipped 2026-05-24 | cascor [PR #302](https://github.com/pcalnon/juniper-cascor/pull/302) (`e8dff8e chore(deps): CFG-02 — move sentry-sdk to [observability] optional extra`); `sentry-sdk>=2.0.0` moved from core deps to the `[observability]` extra (matches the canopy precedent that landed earlier the same week). `src/main.py` bootstrap init now lazy-imports inside the `if _sentry_dsn:` block, wrapped in `try/except ImportError` + clear stderr warning. New 4-case source-level regression suite at `src/tests/unit/test_cfg_02_sentry_sdk_optional.py`. **BREAKING for Sentry users**: deployments must switch from `pip install juniper-cascor` → `pip install juniper-cascor[observability]` (or `juniper-cascor[all]`, which already includes `[observability]`) |
| **CFG-06** | §20 | ✅ shipped 2026-05-23 | cascor-worker [PR #86](https://github.com/pcalnon/juniper-cascor-worker/pull/86) (`052bce0 fix(CFG-06): converge env vars onto JUNIPER_CASCOR_WORKER_* prefix via juniper-config-tools`); preceded by the [juniper-config-tools PyPI migration plan](./JUNIPER_2026-05-22_JUNIPER-ML_CONFIG-TOOLS-PYPI-MIGRATION-PLAN.md) (Waves 0/1/2 via juniper-ml [#318](https://github.com/pcalnon/juniper-ml/pull/318) / [#320](https://github.com/pcalnon/juniper-ml/pull/320) / release [`juniper-config-tools-v0.1.0`](https://github.com/pcalnon/juniper-ml/releases/tag/juniper-config-tools-v0.1.0)). Roadmap text said 13 env vars; actual count was 15 (coordinate correction folded into the design doc + plan) |
| **CFG-09** | §20 | ✅ shipped 2026-05-24 | canopy [PR #318](https://github.com/pcalnon/juniper-canopy/pull/318) (`0f05496 fix(CFG-09): default audit_log_path to logs/audit.log (was /var/log/canopy)`); `Settings.audit_log_path` default changed from `/var/log/canopy/audit.log` (root-only — crashed non-root deployments at `src/audit_log.py:51` mkdir or `:53` `TimedRotatingFileHandler` open) to CWD-relative `logs/audit.log`. Matching parameter default in `src/audit_log.py:27` updated in lockstep. Production deployments continue to override via `JUNIPER_CANOPY_AUDIT_LOG_PATH`. Pinned by new 5-case source-level regression suite at `src/tests/regression/test_cfg_09_audit_log_default.py`. No `Settings` model_validator added — `audit_log.py:51` already mkdir's the parent dir, so it would be redundant. XDG state-home (`~/.local/state/canopy/audit.log`) is a deferred follow-up |
| **API-06** | §21 | ✅ shipped 2026-05-24 | cascor-client [PR #65](https://github.com/pcalnon/juniper-cascor-client/pull/65) (`ffa3520 feat(ws): API-06 — add on_candidate_progress handler + WS_MSG_TYPE constant`); new `WS_MSG_TYPE_CANDIDATE_PROGRESS: str = "candidate_progress"` constant in `juniper_cascor_client/constants.py` + matching `CascorTrainingStream.on_candidate_progress(callback)` registration method (and parallel `FakeCascorTrainingStream.on_candidate_progress(callback)` in the `testing/` subpackage). Pure-additive change — closes the silent-frame gap where cascor's `create_candidate_progress_message` broadcasts (`CandidateProgressEnvelope` in `juniper-cascor-protocol`) had no consumer-registered handler. New `test_on_candidate_progress_callback` regression in `tests/test_fake_ws_client.py`. Roadmap §8001-8013 prescribed the exact code shipped here. Cross-references **XREPO-17** (`§7985`) — also closed |
| **CFG-07** | §20 | 🟢 partially shipped (2026-05-25 audit) | juniper-deploy `README.md` lines 86 / 102 / 119 already document the host:8201↔container:8200 mapping in the topology diagram and env-var table (e.g. row `CASCOR_PORT \| juniper-cascor \| 8200 \| Internal container port`). **Missing**: the roadmap-recommended inline comment in `juniper_cascor_client/constants.py:22-23` explaining the default-port choice (`DEFAULT_BASE_URL: "http://localhost:8200"` targets direct non-Docker connections; Docker users connect on `:8201`). Closing the item fully needs a tiny cascor-client docs comment PR (~3 lines) — deferred follow-up |
| **CFG-08** | §20 | ✅ shipped 2026-05-25 | juniper-ml [PR #326](https://github.com/pcalnon/juniper-ml/pull/326) (`6d24c08 docs(CFG-08): document rate-limit defaults split in REFERENCE.md`); new "Rate Limiting Defaults" subsection under "Ecosystem Compatibility" in `docs/REFERENCE.md`. Confirmed the actual split during the audit: juniper-data ships `rate_limit_enabled=True`, juniper-cascor + juniper-canopy ship `False`; per-minute threshold is uniform at 60 req/min across all three. Documents the rationale (data is the higher-risk public-shaped surface) plus the per-service `JUNIPER_<SERVICE>_RATE_LIMIT_ENABLED` / `_REQUESTS_PER_MINUTE` env-var overrides. Per-service AGENTS.md cross-references deferred |
| **API-03** | §21 | ✅ shipped (de-facto, 2026-05-25 audit) | canopy `src/demo_mode.py:1426` `start(reset: bool = True)` defaults to `True`, calling `Command.RESET` before `START`. `src/backend/training_state_machine.py:243-254` `_handle_reset()` calls `_update_status_and_output(" → Stopped (RESET)")` which **unconditionally** sets `_status = TrainingStatus.STOPPED` regardless of source state — so RESET from FAILED or COMPLETED works fine, and the default-true `start(reset=True)` produces the FAILED/COMPLETED → STOPPED → STARTED auto-reset behavior the roadmap wanted. Implemented as default-parameter dispatch rather than a TRANSITIONS dict (which is what the original §2.2 audit hunted for); functionally equivalent. No new PR work |
| **API-04** | §21 | ✅ shipped (de-facto under **XREPO-05** / Phase 4D, 2026-05-25 audit) | `juniper_cascor_client/testing/constants.py:49-53` `STATE_IDLE: str = "STOPPED"` / `STATE_TRAINING: str = "STARTED"` / `STATE_PAUSED: str = "PAUSED"` / `STATE_COMPLETE: str = "COMPLETED"` — the FakeClient state values now match the server-canonical uppercase FSM names that the canonical `TRAINING_STATE_*` constants in `juniper_cascor_client/constants.py:132-136` exposed. Comment at line 41-48 of `testing/constants.py` explicitly cites Phase 4D as the alignment moment. Original §2.2 audit description ("No `fake_client.py` found") was a misread — the file lives at `juniper_cascor_client/testing/fake_client.py` (testing/ subpackage, not package root); the real XREPO-14 issue (state-vocabulary mismatch) was resolved by XREPO-05 / Phase 4D with no explicit `API-04` commit |
| **API-05** | §21 | ⏳ confirmed TODO (2026-05-25 audit bucket shift) | cascor shipped the `ResponseEnvelope` shape via API-09. **juniper-data** still emits raw `JSONResponse({"detail": ...})` in `juniper_data/api/app.py:138, 146`. **juniper-canopy** uses raw `JSONResponse({"error": ...})` at ~10 sites in `src/main.py` (e.g. lines 1014, 1026, 1038, 1051, 1059, 1062, 1087, 1102) plus a structured `ResponseEnvelope.model_dump()` 500 path at line 328. Real two-service envelope migration needed; roadmap's Approach A is a phased rollout. Warrants its own design doc before scoping |

Rolled-up totals after this status pass (with the 2026-05-25 audit-and-batch-ship folded in):

| Bucket | v7.0.1 | v7.0.2 status pass | Δ |
|---|---|---|---|
| §20 (Configuration/Deps) — open | 13 | 0 confirmed TODO + 0 ambiguous + 1 partial (CFG-07) | −12 (6 silently shipped + 5 explicitly shipped via CFG-06 / CFG-01 / CFG-02 / CFG-09 / CFG-08 + 1 partial via CFG-07) |
| §21 (API/Protocol) — open | 10 | 1 confirmed TODO (API-05) + 0 ambiguous | −9 (4 silently shipped + 1 explicitly shipped via API-06 + 2 de-facto shipped via API-03 / API-04 + 1 reclassified ambiguous → confirmed TODO via API-05) |
| Newly catalogued shipped | n/a | 15 items (CFG-12, CFG-13, CFG-14, CFG-16, API-01, API-02, API-08, **CFG-06**, **CFG-01**, **CFG-02**, **CFG-09**, **CFG-08**, **API-06**, **API-03**†, **API-04**†) | +15 |
| Items needing deeper investigation | n/a | 0 (all 5 resolved: API-03 / API-04 → de-facto shipped, CFG-08 → shipped, CFG-07 → partial, API-05 → confirmed TODO) | −5 (audit closed) |

† = shipped *de-facto* under a different tag (XREPO-05 / Phase 4D for API-04; default-parameter dispatch + unconditional RESET → STOPPED in the canopy state machine for API-03) — no explicit per-ID commit, no further PR needed.

**Pattern observation**: the v7 roadmap's CFG-XX / API-XX entries fall into three classes that were not previously distinguished — (a) shipped under their own ID and never marked, (b) shipped under a different tag (XREPO-* / CC-* / unrelated PR) so the original ID was orphaned, (c) genuinely outstanding. The audit hit-rate (~67% already-done at the start of the pass, ~95% closed by the end of the 2026-05-25 batch) suggests the next CFG/API item should always be re-verified against current code before any worktree is created — formalised as a rule in `juniper-ml/CLAUDE.md` already (roadmap staleness, ~50% stale).

All ambiguous items from the original audit are now resolved. The two remaining work items in §20 + §21 are:

- **CFG-07** (partial) — needs the ~3-line cascor-client `constants.py:22-23` comment to close fully.
- **API-05** (confirmed TODO) — needs a cross-service envelope-migration design doc before implementation (juniper-data + juniper-canopy each have multiple raw `JSONResponse` sites; phased rollout per the roadmap's Approach A).
...
(Phase 2E, juniper-cascor PR [#141](https://github.com/pcalnon/juniper-cascor/pull/141)) — Applied Approach A: wired `create_topology_message()` into `_install_grow_network_hook → monitored_grow` in `src/api/lifecycle/manager.py`.
Whenever new hidden units are installed, a `topology` envelope (`hidden_units`, `input_size`, `output_size`, `event="cascade_add"`) is broadcast via `_ws_manager.broadcast_from_thread`.
Dashboards now receive real-time topology updates on every cascade event.
...
✅ **Implemented 2026-04-25**
(Phase 2E, juniper-cascor PR [#141](https://github.com/pcalnon/juniper-cascor/pull/141)) — Applied Approach A with one extension: removed `# Version: …` / `Version: …` header lines from 65 production+test files.
Replaced the runtime `_API_VERSION` literal in `src/api/app.py` and the `juniper_version` HDF5 attribute literal in `src/snapshots/snapshot_serializer.py` with `importlib.metadata.version("juniper-cascor")` (with a `0.0.0-dev` fallback when the package is not installed).
`pyproject.toml` is now the single source of truth for the version string; remaining version-string drift was caught and removed.
Note: ipynb checkpoints and `scripts/backups/` artifacts were intentionally left untouched.
...
✅ **Implemented 2026-04-25**
(Phase 2E, juniper-cascor PR [#141](https://github.com/pcalnon/juniper-cascor/pull/141)) — Applied Approach A: added `TrainingMonitor.on_phase_change(phase)` plus a `phase_change` callback slot, removed all three manual `monitor.current_phase = "..."` assignments in `manager.py`, and wrapped `TrainingStateMachine.set_phase` via a new `_install_phase_tracker` helper (restored cleanly via `_restore_original_methods`).
Initial OUTPUT propagation is performed explicitly after `Command.START` because `_handle_start` sets `self._phase` directly without routing through `set_phase`.
The state machine is now the single source of truth for `current_phase`; drift between the FSM and the monitor is no longer possible.
...
✅ **Implemented 2026-04-25**
(Phase 2C, juniper-cascor PR [#140](https://github.com/pcalnon/juniper-cascor/pull/140)) — Applied Approach A in `RateLimiter`: added `_maybe_cleanup()` that runs every `_CLEANUP_INTERVAL = 100` `check()` calls, evicts buckets older than `2 * window_seconds`, and enforces a hard cap of `_MAX_ENTRIES = 10_000` (oldest by `window_start` dropped first).
`reset()` clears the cleanup tick counter for clean test isolation.
...
✅ **Implemented 2026-04-25**
(Phase 2C, juniper-cascor PR [#140](https://github.com/pcalnon/juniper-cascor/pull/140)) — Applied Approach A: added `_maybe_full_cleanup()` to `HandshakeCooldown` that runs every `CLEANUP_EVERY_N = 50` `record_rejection()` calls and removes IPs whose timestamps are all older than `2 * window_sec`.
Per-IP pruning logic preserved; global cleanup now collects non-blocked stragglers.
...
✅ **Implemented 2026-04-25**
(Phase 2C, juniper-cascor PR [#140](https://github.com/pcalnon/juniper-cascor/pull/140)) — Applied Approach A: replaced `body = await request.body()` on the no-Content-Length path with `async for chunk in request.stream()` plus an early 413 abort once cumulative bytes exceed `_max_bytes`.
Body is cached on `request._body` so downstream FastAPI handlers can still read it (Starlette convention).
The middleware now matches the streaming intent claimed by its docstring and closes the SEC-08 partial reopening.
...
(Phase 2A, juniper-cascor PR [#138](https://github.com/pcalnon/juniper-cascor/pull/138)) — See ROBUST-01 for the full implementation summary.
...
✅ **Implemented (Phase 3D), 2026-04-27**
juniper-canopy branch `concurrency/phase-3d-bug-cn-01-perform-reset`. `DemoMode._perform_reset()` now applies all three transitions (`self.is_running = False`, `self._stop.clear()`, `self._pause.clear()`) inside a single `with self._lock:` block.
Pre-fix the lock covered only the `is_running` write, so a reader could observe `is_running == False` while `_stop` was still set, leaving the next start() racing against a stale stop signal that gets cleared a moment later.
Verified by `src/tests/unit/test_demo_mode_perform_reset.py::TestPerformResetSourceLevel::test_perform_reset_holds_lock_across_event_clears` (AST-based source check that fails on the pre-fix split-lock and passes after).
...
✅ **Implemented (Phase 3C) 2026-04-26**
Added juniper-canopy branch `concurrency/phase-3c-canopy-thread-safety`.
`WebSocketManager.__init__` now allocates `self._connections_lock = threading.Lock()`;
    every `active_connections`/`connection_metadata` mutation site (`connect`, `disconnect`, `send_personal_message`, `broadcast`, `shutdown`) and
    every observer (`broadcast_from_thread` early-out, `get_connection_count`, `get_connection_info`, `get_statistics`) now executes
    inside `with self._connections_lock:` — readers snapshot the set under the lock and iterate the snapshot outside it,
    so the previous `RuntimeError: Set changed size during iteration` is impossible.
`disconnect()` releases the lock before invoking `_decrement_ip_count` to avoid nesting `_ip_lock` under `_connections_lock`.
Verified by `src/tests/unit/test_websocket_manager_thread_safety.py::TestActiveConnectionsThreadSafety` (concurrent disconnect-vs-snapshot + shutdown snapshot tests).
...
✅ **Implemented (Phase 3C) 2026-04-26**
Added juniper-canopy branch `concurrency/phase-3c-canopy-thread-safety`.
`WebSocketManager.broadcast()` now performs the early-out check, the `self.message_count += 1` increment,
    and the connection-set snapshot inside a single `with self._connections_lock:` block
    (the same lock added for BUG-CN-09).
Per-connection `messages_sent` bookkeeping in `send_personal_message` and `broadcast` also moved under the lock.
Verified by `src/tests/unit/test_websocket_manager_thread_safety.py::TestMessageCountAtomicity::test_broadcast_message_count_no_lost_updates`
    (64 concurrent broadcasts to a seeded connection must produce exactly 64 increments).
...
|----------|-------------------|---------------------------------------------------------------------------|----------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CAN-000  | Meta Param Menu   | Periodic updates pause when Apply Parameters button active                | P2       | 🔴 Open (S) — pause logic on `fast-update-interval`/`slow-update-interval` while Apply is in flight                                                                     |
| CAN-001  | Training Metrics  | Training Loss time window toggle/dropdown                                 | P3       | ✅ Implemented — display-mode toggle + window-selectable Training Loss view in `metrics_panel.py`                                                                       |
| CAN-002  | Training Metrics  | Custom rolling time window for Training Loss graph                        | P3       | ✅ Implemented — `metrics_panel.py:373` `dbc.Input(id=…-window-size, type="number", min=10, max=1000, step=10)` exposes the custom duration; wired to `display-mode-store` at `:1046` |
| CAN-003  | Training Metrics  | Retain candidate pool data per node addition; expandable "Previous Pools" | P3       | ✅ Implemented — `candidate_metrics_panel.py` (`pool-history-store` + `render_pool_history` expandable cards), Sprint 6 (commit `b06c45f`)                              |
| CAN-004  | Meta Param Tuning | New Tab for all exposed meta parameters                                   | P3       | ✅ Implemented — Parameters tab in `dashboard_manager.py` `visualization-tabs` (Sprint 6, commit `b06c45f`)                                                              |
| CAN-005  | Meta Param Tuning | Pin/Unpin meta params from Tuning Tab to left side menu                   | P3       | 🔴 Open (M) — Parameters tab is read-only today; needs pin checkbox + persistent state + sidebar conditional rendering                                                  |
| CAN-006  | Meta Param Tuning | Network train epoch count parameter                                       | P3       | ✅ Implemented — `nn-max-total-epochs-input` exposed; `max_epochs` accepted by `TrainingParams` (`api/models/training.py:46`)                                            |
| CAN-007  | Meta Param Tuning | Candidate pool training epoch count parameter                             | P3       | ✅ Implemented — `cn-training-iterations-input` exposed at `dashboard_manager.py:1050`; `cascor_service_adapter.py:588` aliases `cn_training_iterations` → `candidate_epochs`; included in apply-params flow at `:2313` / `:2402` |
| CAN-008  | Meta Param Tuning | Candidate pool node count parameter                                       | P3       | ✅ Implemented — `cn-pool-size-input` + `candidate_pool_size` (`dashboard_manager.py:988`)                                                                              |
| CAN-009  | Meta Param Tuning | Correlation threshold parameter                                           | P3       | ✅ Implemented — `cn-correlation-threshold-input` + `correlation_threshold` (`dashboard_manager.py:999`)                                                                |
| CAN-010  | Meta Param Tuning | Optimizer type meta parameter                                             | P3       | 🚧 Blocked — TrainingParams has no `optimizer_type` field; cascor hardcodes a single optimizer. Cross-repo work: extend `TrainingParams` + cascor optimizer factory      |
...
| GAP-WS-25     | **P1**   | WebSocket-health-aware polling toggle — both WS+REST run simultaneously causing duplicates   | ✅ Complete — metrics half closed by GAP-WS-16 ([juniper-canopy #195](https://github.com/pcalnon/juniper-canopy/pull/195)); topology half via [juniper-canopy #196](https://github.com/pcalnon/juniper-canopy/pull/196). Raw-topology intentionally remains REST-only (no WS source, rationale comment in dashboard_manager.py). |
...
| GAP-WS-18     | **P1**   | Topology message can exceed 64 KB silently causing connection teardown                       | ✅ Complete — [juniper-cascor #153](https://github.com/pcalnon/juniper-cascor/pull/153) (server-side size guard + `chunked_message` protocol with per-chunk seq + replay) + [juniper-canopy #197](https://github.com/pcalnon/juniper-canopy/pull/197) (client reassembler with bounded in-flight groups). Each chunk lands in the replay buffer with its own seq, so resume-on-reconnect reorders chunks naturally. |
...
✅ Complete (2026-04-29). Metrics half closed by GAP-WS-16 ([juniper-canopy #195](https://github.com/pcalnon/juniper-canopy/pull/195) — `_update_metrics_store_handler` gates on `connected && metricsReceived`); topology half closed by [juniper-canopy #196](https://github.com/pcalnon/juniper-canopy/pull/196) (`_update_topology_store` gates on `connected && topologyReceived`, mirroring the metrics pattern). Raw-topology intentionally remains REST-only — cascor doesn't broadcast raw weight matrices on `/ws/training`, only the structural `topology` event from `cascade_add`. Rationale comment lives in `dashboard_manager.py` so future contributors don't 'fix' the absence.
...
✅ Complete (2026-04-29). [juniper-cascor #153](https://github.com/pcalnon/juniper-cascor/pull/153) shipped Approach A (chunking, not compression — gives lossless reconstruction with no client-side zlib dependency). `WebSocketManager._maybe_chunk_message` splits oversized broadcasts into `chunked_message` envelopes (default threshold 60_000 bytes, chunk payload 32_000 bytes); each chunk gets its own seq + replay-buffer slot so resume-on-reconnect reorders them naturally. Client reassembler in [juniper-canopy #197](https://github.com/pcalnon/juniper-canopy/pull/197) accumulates by `chunk_id`, JSON.parses on completion, recurses through `_handleMessage` so the original envelope flows through normal dispatch. Bounded to 8 in-flight groups; oldest evicted at cap. Two new `transport_stats` counters surface chunk activity (`messages_chunked_total`, `chunks_emitted_total`).
...
🟡 In Review (2026-04-29). [juniper-cascor #154](https://github.com/pcalnon/juniper-cascor/pull/154) closes the all-or-nothing-semantics gap surfaced during the 2026-04-28 audit. The race itself was already closed by `_training_lock`; the PR adds capture-then-revert atomicity: snapshot pre-call values for every applicable key, attempt the setattr loop, and on any exception walk the applied list in reverse and restore each key. Re-raises the original exception so callers see the failure. Defensive guard for future validating property setters — no setter currently raises.
...
✅ **Implemented 2026-04-24 (Phase 4B)**
`juniper_cascor_client/constants.py` now defines `RETRYABLE_STATUS_CODES = [429, 502, 503, 504]`.
The change is one line plus a longer header comment explaining why 503 (service restart/deploy) and 429 (rate limit) are now retryable.
A new regression suite `tests/test_retry_policy.py` pins the allow-list in both directions (canonical transients retried, non-transient 4xx/5xx not) and asserts the `Retry` adapter mounted on `session` reflects the constant end-to-end.
The existing `test_service_unavailable_503` test was updated to mount a retry-free `HTTPAdapter` for that one case so it continues to exercise the `JuniperCascorServiceUnavailableError` mapping path.
All 251 cascor-client tests pass.
...
✅ **Implemented 2026-04-24 (Phase 4B)**
`JuniperDataClient.create_dataset()` and `FakeDataClient.create_dataset()` now accept `tags: Optional[List[str]]` and `ttl_seconds: Optional[int]`.
Both are forwarded to the server's `CreateDatasetRequest` (real client) and persisted in `meta` (fake).
The fake enforces the server's `ge=1` Pydantic bound on `ttl_seconds` so tests catch misuse.
New regression suite `tests/test_create_dataset_tags_ttl.py` covers POST-body shape (via mocked `_request`),
    fake-client round-trip through `get_dataset_metadata`, validation of zero / negative TTL,
    list-aliasing safety, and JSON serializability.
All 183 data-client tests pass.
...
✅ **Implemented 2026-04-24 (Phase 4B)**
`juniper_data_client/constants.py` now defines `RETRY_ALLOWED_METHODS = ["HEAD", "GET", "PUT"]` per RFC 9110 §9.2.2 (idempotent-only).
POST, PATCH, and DELETE were removed to prevent duplicate dataset creation (on POST) and repeated side-effects (on DELETE) when a transient 5xx retried a request that had already been applied server-side.
The CHANGELOG documents this as an intentional behavior change; callers that need retry for mutations must layer their own idempotency.
New regression suite `tests/test_retry_policy.py` pins the allow-list both ways (idempotent allowed, non-idempotent blocked) and verifies the `Retry` adapter mounted on the session reflects the constants end-to-end.
...
rm notes/development/JUNIPER_2026-04-20_JUNIPER-CANOPY_DASHBOARD-AUGMENTATION-PLAN.md
```
...
| Phase  | Items                                                  | Scope | Description                                                                                                             |
|--------|--------------------------------------------------------|-------|-------------------------------------------------------------------------------------------------------------------------|
| 3A ✅  | CONC-04/BUG-JD-10 ✅, CONC-07/BUG-CN-11 ✅             | 2×S   | Async event loop blocking (closed via Phase 1D PR #45 SEC-04 shared fix), state mutation                                |
|        |                                                        |       | (Implemented 2026-04-26, juniper-canopy concurrency/phase-3a-track-3-conc-07-bug-cn-11)                                 |
| 3B ✅  | CONC-01 ✅, CONC-02/BUG-CC-16 ✅, CONC-03/BUG-CC-17 ✅ | 3×S   | Per-IP race (Implemented 2026-04-26, juniper-canopy concurrency/phase-3b-conc-01-per-ip-race), broadcast throttle +     |
|        |                                                        |       | split-lock (Implemented 2026-04-26, juniper-cascor concurrency/phase-3b-conc-02-conc-03-broadcast-and-metrics)          |
| 3C ✅  | BUG-CN-09 ✅, BUG-CN-10 ✅, CONC-08 ✅, CONC-09 ✅     | 4×S   | Thread-safe sets + atomic counters (Implemented 2026-04-26, juniper-canopy concurrency/phase-3c-canopy-thread-safety),  |
|        |                                                        |       | is_running consistency (same canopy branch), fire-and-forget startup tasks (Implemented 2026-04-26,                     |
|        |                                                        |       | juniper-cascor concurrency/phase-3c-conc-09-startup-task-references)                                                    |
| 3D ✅  | CONC-10 ✅, CONC-12/BUG-JD-11 ✅, BUG-CN-01 ✅         | 3×S   | Health monitor race (Implemented 2026-04-27, juniper-cascor concurrency/phase-3d-conc-10-health-monitor-race),          |
|        |                                                        |       | access-count TOCTOU (Implemented 2026-04-27, juniper-data concurrency/phase-3d-conc-12-record-access-toctou),           |
|        |                                                        |       | reset race (Implemented 2026-04-27, juniper-canopy concurrency/phase-3d-bug-cn-01-perform-reset) — **Track 3 complete** |
...
| 4D ✅  | XREPO-04, XREPO-05, XREPO-07/XREPO-08            | 3×M   | Protocol constants, state names, WS message format (Implemented 2026-04-26, |
|        |                                                  |       | juniper-cascor-client PR #25 + juniper-cascor-worker PR #34)                |
| 4E ✅  | CC-04..CC-07, CW-02..CW-08                       | 8×S-M | Client missing methods, worker improvements (Implemented 2026-04-26,        |
|        |                                                  |       | juniper-cascor-client PR #25 + juniper-cascor-worker PR #34 +               |
|        |                                                  |       | juniper-data-client PR #36; CW-05 Approach B interim — see follow-up)       |
...
| 6E    | CAS-002..009 + 6 deferred CAN items     | 5 sprints / 19 PRs | CasCor algorithm enhancements + cross-repo CAN-* | 🔴 Outstanding — three design docs landed 2026-04-29: [`JUNIPER_2026-05-02_JUNIPER-ECOSYSTEM_PHASE-6E-DESIGN.md`](JUNIPER_2026-05-02_JUNIPER-ECOSYSTEM_PHASE-6E-DESIGN.md) (master plan), [`JUNIPER_2026-04-29_JUNIPER-CASCOR_CAN-013-INTEGRATION-MODE-DESIGN.md`](JUNIPER_2026-04-29_JUNIPER-CASCOR_CAN-013-INTEGRATION-MODE-DESIGN.md) (full integration modes incl. weighted_ensemble unit type), [`JUNIPER_2026-04-29_JUNIPER-ECOSYSTEM_PHASE-6E-MULTI-NETWORK-DESIGN.md`](JUNIPER_2026-04-29_JUNIPER-ECOSYSTEM_PHASE-6E-MULTI-NETWORK-DESIGN.md) (multi-network refactor for CAS-008/009/CAN-021). Sprint plan: A (5 wire-through PRs, ~1 wk) → B (snapshot replay, 1 PR) → **C (full CAN-013 incl. ensemble unit, 5 PRs, ~1.5 wk)** → D (CAS-005 cleanup, 1 PR) → E (multi-network, 7 PRs, ~2 wk). 3 of 6 deferred CAN items are wire-through (CAN-010 / 011 / 014); CAN-013 expanded to full weighted_ensemble per user direction. |
...
**Track 6 progress (2026-04-29 — deep design pass)**: Phases 6A, 6B, 6C, 6D complete (4/6). 6F drained (only conditional Phase E..H backpressure pump tasks left). **6E now has three design docs**: [`JUNIPER_2026-05-02_JUNIPER-ECOSYSTEM_PHASE-6E-DESIGN.md`](JUNIPER_2026-05-02_JUNIPER-ECOSYSTEM_PHASE-6E-DESIGN.md) is the master plan, [`JUNIPER_2026-04-29_JUNIPER-CASCOR_CAN-013-INTEGRATION-MODE-DESIGN.md`](JUNIPER_2026-04-29_JUNIPER-CASCOR_CAN-013-INTEGRATION-MODE-DESIGN.md) covers the full weighted_ensemble unit type (per user direction), and [`JUNIPER_2026-04-29_JUNIPER-ECOSYSTEM_PHASE-6E-MULTI-NETWORK-DESIGN.md`](JUNIPER_2026-04-29_JUNIPER-ECOSYSTEM_PHASE-6E-MULTI-NETWORK-DESIGN.md) covers the registry-pattern refactor for CAS-008/009/CAN-021. Three "blocked" CAN items (CAN-010 / 011 / 014) turn out to be wire-through. Plan: 5 sprints / 19 PRs total — A (5 wire-throughs) → B (snapshot replay) → C (full CAN-013) → D (CAS-005 cleanup) → E (multi-network XL). Adjacent **Network Evolution** ([juniper-canopy #203](https://github.com/pcalnon/juniper-canopy/pull/203)) already covers part of CAN-021's "see multiple network states" intent without waiting for Sprint E.
...
**Source**: 2026-05-05 multi-agent audit (see [`JUNIPER_2026-05-05_JUNIPER-ECOSYSTEM_ROADMAP-AUDIT.md`](./JUNIPER_2026-05-05_JUNIPER-ECOSYSTEM_ROADMAP-AUDIT.md) §5) cross-referenced against `git log --since="2026-04-23"` per repo. Total shipped: **654 commits across 8 repos in 12 days**.
...
- [juniper-ml #212](https://github.com/pcalnon/juniper-ml/pull/212) — `JUNIPER_2026-05-04_JUNIPER-ECOSYSTEM_PHASE-6E-DEFERRED-CAN-015GH-DESIGN.md` annotated with delivery state
