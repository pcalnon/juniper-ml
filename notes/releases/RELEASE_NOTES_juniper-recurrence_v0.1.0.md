# juniper-recurrence v0.1.0 — Release Notes (archived)
...
First release of `juniper-recurrence`, the FastAPI + CLI application that serves the Δt-native LMU
...
> **Status:** ALPHA — initial research release. Single-worker, in-process state; synchronous
...
- `build_app()` wires `create_app(routers=…)` plus the canonical
  `RequestBodyLimit → SecurityHeaders → Security` middleware stack; module-level `app` for uvicorn.
...
- `data.py`: `juniper-data-client` → 3-D NPZ (`equities_seq`) → model kwargs
