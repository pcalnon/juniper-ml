...
| Task | Source | Details | Effort |
|------|--------|---------|--------|
| Refactor tests away from deprecated convergence code | Audit | 5 test files reference `_should_add_cascade_unit`, `CASCADE_COOLDOWN_EPOCHS`, `_cascade_cooldown_remaining`. Update tests to use production code paths, then delete deprecated code | 1-2 hr |
...
### Step 3.1: Async/Sync Boundary Tests (P1)

| Task | Source | Details | Effort |
|------|--------|---------|--------|
| Add dedicated async/sync boundary test suite | CAN-HIGH-003 | Test `run_in_executor` usage, `nest_asyncio` interaction, sync methods calling async internals, event loop threading | 2-3 hr |

### Step 3.2: Real Backend Path Coverage (P1)

| Task | Source | Details | Effort |
|------|--------|---------|--------|
| Add gated real-backend tests | CAN-HIGH-004 | Tests exercising real CasCor code paths, gated behind `CASCOR_BACKEND_AVAILABLE=1`; use `FakeCascorClient` for unit-level paths | 3-4 hr |
| Main.py 95% coverage | CAN-HIGH-008 | Cover error handling branches, real-backend startup, fallback-to-demo edge cases | 2 hr |
