...
> historical build record for the WS-6 golden gate. See `JUNIPER_2026-06-21_JUNIPER-ECOSYSTEM_DOCS-REALITY-AUDIT.md`.

> **What this is.** A build plan for the **golden / snapshot regression suite** that must exist in `juniper-cascor` *before* the WS-6 cutover (refactoring cascor onto `juniper-service-core` / `juniper-model-core`). It is **OUT-12** in [`JUNIPER_2026-06-17_JUNIPER-ECOSYSTEM_PLATFORM-ENVIRONMENT-STATE-AND-ROADMAP.md`](JUNIPER_2026-06-17_JUNIPER-ECOSYSTEM_PLATFORM-ENVIRONMENT-STATE-AND-ROADMAP.md) and the explicit trigger-gate for WS-6 in the [middleware refactor plan](JUNIPER_2026-05-31_JUNIPER-ECOSYSTEM_MODEL-MIDDLEWARE-REFACTOR-DESIGN-AND-PLAN.md) §2.7 / §3.4 / Part 8. The suite is captured on `origin/main` *now* (as insurance), then re-run post-refactor; WS-6 is accepted only if it — plus the model-core conformance kit — stays green.
>
