...
| V17 | juniper-data            | lockfile-update.yml | update                                              | secret missing (CROSS_REPO_DISPATCH_TOKEN)                         | P2       | G-CONFIG   | **fixed (pending verification)** | secret provisioned 2026-05-02 (see notes/V17_V18_CROSS_REPO_DISPATCH_TOKEN_SETUP_2026-05-02.md). E2E verification will land when the next Dependabot `dependabot/pip/**` PR triggers `lockfile-update.yml` and the post-update commit re-triggers ci.yml. |
| V18 | juniper-canopy          | lockfile-update.yml | update                                              | secret missing (CROSS_REPO_DISPATCH_TOKEN)                         | P2       | G-CONFIG   | **fixed (pending verification)** | secret provisioned 2026-05-02; same verification path as V17. |
...
| V38 | juniper-cascor          | ci.yml              | unit-tests (post-V37)                               | (a) `test_grow_network_adds_hidden_unit` + (c) 3                    | P2       | G-CODE     | **partially fixed** | cascor `cd86a90` — (b) `os.sched_getaffinity` mocks now use         |
...
|     |                         |                     |                                                     |                                                                    |          |            |              | **V38a/V38c remain deferred** — `grow_network` "Training results    |
|     |                         |                     |                                                     |                                                                    |          |            |              | are None" looks like genuine product/test-data interaction.          |
