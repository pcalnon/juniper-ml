...
|     |                         |                     |                                                     |                                                                    |          |            |              | gate.                                                                |
| V34 | juniper-canopy          | ci.yml              | unit-tests (matrix; 4 legs)                         | (a) `Duplicated timeseries in CollectorRegistry` (prom metric       | P2       | G-CODE     | **fixed**    | canopy `b33f81f` — (a) `_ensure_canopy_metrics` now adopts existing |
...
| V35 | juniper-cascor          | ci.yml              | unit-tests (matrix; 4 legs)                         | (a) `test_api_middleware.TestRequestBodyLimitMiddleware` chunked-   | P2       | G-CODE     | **fixed**    | cascor `0a8bf7b` — (a) Starlette 1.0 dropped silent `_body` cache;   |
|     |                         |                     |                                                     | body 422 ≠ 200; (b) `test_app_startup_tasks` log-capture            |          |            |              | middleware now installs a one-shot receive replayer in addition to  |
|     |                         |                     |                                                     | (assert 0 == 1); (c) `test_coordinator_health_monitor_race`         |          |            |              | the `_body` cache; (b) test now attaches caplog handler directly to |
|     |                         |                     |                                                     | CONC-10 race on stale-worker dereg; (d)                            |          |            |              | `juniper_cascor.api` (independent of propagation state); (c) CONC-10|
|     |                         |                     |                                                     | `test_lifecycle_manager` epochs_max default 1e11 vs 200 drift      |          |            |              | `get_next_assignment` now checks registry presence under the same   |
|     |                         |                     |                                                     |                                                                    |          |            |              | lock that `_check_stale_workers` deregisters under; (d) lifecycle   |
|     |                         |                     |                                                     |                                                                    |          |            |              | training-limit defaults restored to spec values per PR #122.         |
