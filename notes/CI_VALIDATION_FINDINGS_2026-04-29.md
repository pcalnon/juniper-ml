...
| V24 | juniper-cascor          | ci.yml              | Lockfile Freshness                                  | dependency (lock drift)                                            | P2       | G-CODE     | **deferred** | still firing on cascor; needs `uv pip compile` against cascor's deps  |
...
| V29 | juniper-cascor          | ci.yml              | unit-tests (matrix; all 4 legs)                     | unit (test_api_health.py::TestProbeDependency::* —                 | P1       | G-CODE     | **deferred** | new finding; appears to be a real product test/code mismatch         |
|     |                         |                     |                                                     | `AttributeError: module 'api.models.health' has no attribute       |          |            |              | introduced after the V01-V03 startup_failure was cleared.            |
|     |                         |                     |                                                     | 'urllib'`)                                                         |          |            |              | Out-of-scope for the alignment effort.                                |
