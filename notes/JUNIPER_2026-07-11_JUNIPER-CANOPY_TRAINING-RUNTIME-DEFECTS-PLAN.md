...
- **Isolated-stack E2E checklist** (the durable, repeatable runbook for the above): [`JUNIPER_2026-07-21_JUNIPER-ECOSYSTEM_ISOLATED-STACK-E2E-CHECKLIST.md`](JUNIPER_2026-07-21_JUNIPER-ECOSYSTEM_ISOLATED-STACK-E2E-CHECKLIST.md) — the bring-up recipe (data 8101 dedicated venv + cascor 8202 + canopy 8051), the control-WS Origin/allowlist pair, per-issue copy-pasteable probes with §13 evidence, and teardown hygiene; encoded (with a side-effect-free `--dry-run`) as `util/isolated_stack.bash` (`--up`/`--down`/`--status`) in juniper-ml (roadmap unit E1).
...
|------------------|--------------------|--------------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| D1               | verified           | juniper-data#220         | 2026-07-18 | E2E 2026-07-18 (isolated 8101): availability map correct; mnist→501+install hint; spiral control 201                                                                                                                                                                                                                                                             |
| D2               | verified           | juniper-data#221         | 2026-07-18 | E2E: [mnist] extra flips available:true; real HF generation (784 features, 512 samples); I-5 full-stack: cascor auto-built 784×10 net and trained                                                                                                                                                                                                                |
| C1               | verified           | juniper-cascor#397       | 2026-07-18 | E2E: null/blank/filled snapshot descriptions → 200 during a live run                                                                                                                                                                                                                                                                                             |
| C2a              | verified           | juniper-cascor#398       | 2026-07-18 | E2E: applied/skipped partition live on the wire; atomic 422 intact                                                                                                                                                                                                                                                                                               |
| C2b              | verified           | juniper-cascor#400       | 2026-07-19 | E2E: /v1/network vs /v1/training/status coherent; derived cap (114000) live on both apply echo and status                                                                                                                                                                                                                                                        |
| C3               | verified           | juniper-cascor#401       | 2026-07-19 | E2E: idle /ws/training pinged ~30 s then closed 1011 + 'Heartbeat timeout…' reason; control-WS origin gate exact (allowlisted connects; disallowed/no-origin 403)                                                                                                                                                                                                |
| C4               | verified           | juniper-cascor#407       | 2026-07-19 | E2E: uvicorn access lines continue after training start; rejected start logs WARNING                                                                                                                                                                                                                                                                             |
| C5               | verified           | juniper-cascor#408       | 2026-07-19 | E2E: 401 rows retained across stop; clear→undo exact round-trip; undo-after-start 409; start_fresh preserved 610 snapshots + discarded history                                                                                                                                                                                                                   |
| C6               | dropped(satisfied) | —                        | 2026-07-20 | E-1 live census: per-epoch emission ALREADY EXISTS (245 metrics frames/100 s, every 25th epoch in output phases; candidate_progress in candidate phases) — nothing to build; N8 consumes the existing stream; reopen only if N8 finds a gap                                                                                                                      |
| C7               | pr-open            | juniper-cascor#419       | 2026-07-21 | phase 1 scalars (f1/precision/recall/ROC-AUC) torch-native, zero new deps; manager-drain seam, core algorithm untouched; additive nullable on history/snapshot/WS; NO protocol release needed; phase-2 curves deferred (N9 owns display)                                                                                                                         |
| CL1              | merged             | juniper-cascor-client#92 | 2026-07-16 | merged 2026-07-13; auto-pong + liveness surface + msg_type warnings + Fake parity; version 0.7.0                                                                                                                                                                                                                                                                 |
| CL2              | merged             | juniper-canopy#453       | 2026-07-20 | floor >=0.7.0 + lockfile; 3 adapter seams onto the client liveness surface; manual pong retired (is_alive-fed relay re-arm); test_stream_liveness (41) now runs in the required unit lane                                                                                                                                                                        |
| N1               | verified           | juniper-canopy#443       | 2026-07-19 | REST backing verified live + the WS-silent Playwright suite green in CI (#449/#450); browser poll liveness CI-covered                                                                                                                                                                                                                                            |
| N2               | verified           | — (direct push)          | 2026-07-19 | E2E: /api/stream_health healthy×3 with documented origin override; /api/state live-first stale:false; per-attempt WARNING reconnect cycle + relay summaries observed                                                                                                                                                                                             |
| N3               | merged             | juniper-canopy#459       | 2026-07-20 | merged 2026-07-20; /api/train/restart route (stop→await→start(staged), E-2 pin); Q3/Q4 modal; demo-FSM terminal auto-reset (BEHAVIOR CHANGE for owner review); start_fresh via client _post (CL2-family seam); granular MODIFY split to N3b                                                                                                                      |
| N3b              | merged             | juniper-canopy#462       | 2026-07-21 | merged 2026-07-21; modal granular MODIFY: 5 staged-dataset fields + 6 CascorPatchBounds-governed params; delegates to N5's extracted _apply_params_via_backend core; re-stage→apply→orchestrate with abort-on-failure; suites green                                                                                                                              |
| N4               | verified           | juniper-canopy#442       | 2026-07-18 | E2E: seam holds (cascor receives ""); detail passthrough works; surfaced the service-mode local-stat 500 → fix canopy#451                                                                                                                                                                                                                                        |
| N5               | merged             | juniper-canopy#460       | 2026-07-20 | merged 2026-07-20; CascorPatchBounds mirror + clamp (found real divergences: canopy MAX_LEARNING_RATE 1.0 vs cascor le=10.0, pool 500 vs 256, patience 500 vs 100000 — drift-pin test added); verbatim rejection toasts; C2a partition surfaced REST+WS; liveness-gated WS leg                                                                                   |
| N6               | merged             | juniper-canopy#458       | 2026-07-20 | merged 2026-07-20; header Step/Hidden-Units relabels; Iteration = grow_iteration/grow_max; phase-qualified Epoch; kind-aware tile holds through candidate freeze; get_status carries the reconciled counters                                                                                                                                                     |
| N7               | pr-open            | juniper-canopy#465       | 2026-07-21 | schema-driven panel (I-7) via new dataset_schema.py; availability gating w/ flag-absent fallback; rank-clarifying hint; U-6 per-type section; non-spiral params ride the generic params channel (nn_dataset_params); NO data-client floor bump (direct httpx); plotter-selector gating + deploy data-service floor = follow-ups                                  |
| N8               | merged             | juniper-canopy#472       | 2026-07-23 | merged 2026-07-22/23; WS-primary tiles/state (O3) + liveness-gated NON-STICKY poll fallback (O1): gate = frame-arrival AGE vs WS_LIVENESS_WINDOW_MS; two-callback split per store (WS-buffer-as-poll-Input re-creates I-1 starvation via Dash chained-no_update skip — found empirically); fixed the latent dead state_change→state bridge handler; suites green |
| N9               | merged             | juniper-canopy#471       | 2026-07-23 | merged 2026-07-22/23; feature code landed on main via a848cac DIRECT PUSH during the session-limit window (no PR; corrupted _FakePhase broke suite collection on main); #471 = repair + changelog; bounded-[0,1] classification plot w/ C7 scalars, gap-aware nullables, eval_metrics surfaced never guessed, WS trace-index contract pinned                     |
| N10              | merged             | juniper-canopy#470       | 2026-07-23 | merged 2026-07-22/23; code landed on main via fa5f32a DIRECT PUSH (no PR/changelog); #470 = changelog-record + the stale worker-pin alignment + N9's byte-identical module repair (MERGE #470 FIRST to un-red main); discovery: cascor registry = remote-only, local pool unexposed (cascor follow-up proposed); honest local/remote hybrid                      |
| E1               | pr-open            | juniper-ml#686           | 2026-07-21 | checklist doc (notes/…ISOLATED-STACK-E2E-CHECKLIST.md) + util/isolated_stack.bash (--up/--down/--status/--dry-run) + 29-test contract suite in CI; §9 pointer + AGENTS.md wiring; corrected two stale claims ([api] extra drift; CASCOR_SERVICE_URL is the deprecated alias)                                                                                     |
| E-1 (experiment) | verified           | —                        | 2026-07-16 | LIVE census (isolated 8202/8101, 100 s): 245 metrics frames (every 25th epoch, output phases), candidate_progress in candidate phases, pings answered per C3; transport counters corroborate exactly                                                                                                                                                             |
| E-2 (experiment) | verified           | —                        | 2026-07-16 | cascor half: start?reset=true vs ACTIVE run → immediate 409 (54 ms), no executor queueing; staged config SURVIVES the refusal                                                                                                                                                                                                                                    |
| E-3 (experiment) | merged             | — (with N2)              | 2026-07-14 | resolved by the N2 implementation: always-release + clientside watchdog                                                                                                                                                                                                                                                                                          |

**2026-07-16 addendum — wave-2 status sweep + experiments.** Dup-guard sweep before wave-2 dispatch found C2b, C3 and CL1 already merged 2026-07-13 (cascor#400, cascor#401, cascor-client#92 = 0.7.0): formalizations of further 2026-07-12 in-flight session work that the owner direct-merged to cascor main (per #401's own scope note — cascor's equivalent of the canopy N1/N2 landing, owner-acknowledged). C4 and C5 task-executors dispatched 2026-07-16. Experiments run live on an isolated stack (cascor 8202 + juniper-data 8101; operator stack untouched): **E-1** — cascor DOES emit `metrics` on `/ws/training` (245 frames in 100 s; every 25th epoch during output phases; `candidate_progress` covers candidate phases; C3's transport counters and emission summary corroborate exactly), so C6's add-emission premise is void and N8 can consume the existing stream; **E-2 (cascor half)** — `start?reset=true` against an ACTIVE run returns an immediate 409 (54 ms, no executor queueing) and the staged dataset config survives the refusal, pinning N3's active-run design as stop → await → start(staged). CL2 is blocked on the owner cutting the cascor-client 0.7.0 GitHub Release (version already merged; latest release is v0.6.0); its canopy half then swaps the adapter's documented `_ws` seams onto the client liveness surface and retires the manual-pong workaround. C4 outcome: the implementation and regression tests were ALREADY on cascor main via a further unrecorded direct push (`a0d27053`, 2026-07-11 — access-log `disable_existing_loggers` fix + root-clobber guard + WARNING on rejected starts + `test_access_log_survival.py`); cascor#407 is the changelog-record PR. New residual: C1 (#397) and C2b (#400) also lack CHANGELOG entries — candidate changelog-record follow-ups for full unit parity. C5 outcome: cascor#408 (CI green) ships the full Q4 contract — additive `start_fresh` on start (snapshot-preserving), `POST /v1/training/metrics/clear` + `/clear/undo` (undo valid until the next start), `metrics_clear_undo_available` on status, retention-by-default for `GET /v1/metrics/history`; canopy N3 and CL2 (client/Fake parity for the new routes) consume these surfaces. Side-findings: two `test_pending_dataset.py` integration tests fail identically on cascor main (staged-generator alias `spirals`→`spiral`, `manager.py:2942` — pre-existing, un-gated by the default CI lane; separate fix warranted); cascor's engine FSM auto-resets from COMPLETED on START (`state_machine.py:171-173`) — the start-from-COMPLETED refusal is canopy's demo FSM only. 2026-07-17 update: C4 (#407) and C5 (#408) MERGED 2026-07-16. The pending-dataset side-finding turned out to be a live MAIN-CI red: cascor's CI/CD Pipeline has been failing on the Quick Integration lane (e.g. the 4f9e72d4 run, 2026-07-14 — masked by the green Scheduled/CodeQL/Golden workflows) because two tests still pinned the pre-#396 untranslated staged-config forwarding; fix = cascor#409 (pr-open; test-only; full lane 172/172 with it).

**2026-07-14 addendum — wave-1 close-out and the N1/N2 landing anomaly.** All five wave-1 PRs merged 2026-07-12 01:10–01:28 UTC (owner-gated). Separately, the N1 implementation (canopy 46f3b44 + 87f7df6) and the complete N2 implementation (canopy c611afb: supervisor half-open detection via real socket state + keepalive probes, relay liveness windows with logged reconnect/backoff and a periodic throughput summary, `StreamHealth` + `GET /api/stream_health` + the badge's upstream degraded-mode dimension, `/api/state` live-first base fields with an honest `stale` fallback, and the E-3 fix: always-release of the apply-in-flight clamp + a clientside watchdog) were pushed DIRECTLY to canopy main 2026-07-12 02:19–02:29 UTC as back-merge states of two session branches (`fix/ungate-metrics-topology-polls`, `fix/relay-supervisor-liveness-and-state-truth`; both now fully contained in main), with no PR, interleaved with six metrics-layout timestamp-churn commits. canopy#443 (merged 2026-07-13) then formalized N1's test surface. Retroactive review 2026-07-14: plan-conformant (all §8 guard rails present — empty-guard, `asyncio.to_thread` event-loop guard, bounded full-history fetch), no blocking defects; residuals: the N2 CHANGELOG entry is missing (N1 has one); client-side ping/pong remains CL1's root fix (the adapter's `_ws` reach-ins are documented CL1 swap seams); and `src/tests/unit/backend/test_stream_liveness.py` importorskips `juniper_cascor_client` — the 510-line N2 liveness suite is **silently skipped in CI's unit job** (which installs `-e .` without the optional `[juniper-cascor]` extra), so it has only ever run in client-equipped local envs (CL1/CL2 should decide its CI home). The same push shipped a UI test whose failures turned canopy main's UI leg red from 2026-07-12 02:29; canopy#449's diagnosis found TWO causes: (a) test-side — the "if not running, click Start" precondition meets CI demo runs converged to fsm COMPLETED (no juniper-data in CI), and START from COMPLETED is refused (`/api/train/start` defaults `reset=False`; the FSM has no COMPLETED→start branch) — fixed with Reset → Start preconditions + dash-4-aware harness; (b) product — the topology store handler's lazy `CascorServiceAdapter` import (for `_transform_topology`) pulls `juniper_cascor_client`, absent by design in client-less installs → `ModuleNotFoundError` → silent `no_update` → a permanently empty Network Topology panel in ANY demo-only install, latent until this test observed the store wire — fixed with a graph-format passthrough that skips the adapter (unit + Playwright regression pins). Source-branch deletion pending owner ack.

**2026-07-18 addendum — wave-1 E2E verification (per the owner's directive).** Full §9 isolated-stack pass (data 8101 in a dedicated venv — JuniperData env untouched — + cascor 8202 + canopy 8051 in service mode; operator's Docker stack never touched): D1, D2, C1, C2a, N4 all → `verified` (row notes carry the evidence), plus **I-5 end-to-end** (canopy-dialect mnist staged on cascor → fetched from isolated juniper-data → 784×10 network auto-created → training ran) and **I-4 end-to-end** (C2b's derived cap `epochs_max=114000` seeds `/api/state` with `stale:false`; full-form apply with the seeded values succeeds through canopy — the evening-502 wholesale-422 class is gone). ONE defect surfaced and resolved: **service-mode `POST /api/v1/snapshots` always 500'd after a successful cascor save** — the route stat()ed a local path the adapter deliberately ignores (cascor stores server-side; canopy's deploy container mounts no snapshot volume), so upstream success was reported as failure in EVERY topology including production Docker; zero test coverage on that branch. Fix + regressions + live 201 re-verify: **canopy#451** (pr-open). Wave-2-pass intel gathered en route: control-WS to cascor returns 403 in the split-host topology (apply's REST fallback engaged cleanly — C3/N2 resilience visible); the 0.6.0-era installed client still emits unrecognized-frame warnings (retired by CL2's floor bump); the plan's old "second None-description seam at main.py:2718" pointer is stale (that region is now the layouts API — treat as resolved-or-moved).

**2026-07-19 addendum — wave-2 E2E verification + the deploy train.** PyPI deploys live: cascor-client 0.7.0, cascor 0.6.0 (`meta.version` 0.6.0 observed live), juniper-data 0.10.0 (releases cut from the merged prep PRs cascor#410/data#230; owner approved the environment gates). Wave-2 isolated-stack pass (data 8101 venv + cascor 8202 + canopy 8051): C2b, C3, C4, C5, N1, N2 all → `verified` (row notes carry evidence). The wave-1 pass's control-WS 403 is fully explained as CONFIG, not defect: cascor's `/ws/control` origin allowlist plus canopy's derived default origin `http://{hostname}:8050` — the settings docstring already documents the split-host remedy (`JUNIPER_CANOPY_CASCOR_WS_ORIGIN` + matching allowlist entry), and with it set the control stream connects and `/api/stream_health` reports healthy across the board. These origin/allowlist requirements and the data-venv bootstrap belong in E1's isolated-stack checklist. Minor engine curiosity recorded (not a unit failure): a `start_fresh` 2×2 vanilla rebuild on the small spirals set ended its run at epoch 0 with empty history — instant convergence/no-candidate path worth an eye during N3's restart work. CL2's canopy half (floor ≥0.7.0, adapter seams onto the client liveness surface, manual-pong retirement, `test_stream_liveness` CI home) is in flight.

**2026-07-20 addendum — wave 2 closed.** CL2 merged (canopy#453): client floor >=0.7.0 with regenerated lockfile, the adapter's three documented seams swapped onto cascor-client 0.7.0's liveness surface (`is_connected` property, passive `is_alive` probing, manual relay pong removed with an `is_alive`-fed 30 s re-arm replacing the ping feed of the liveness clock), and the 41-test N2 stream-liveness suite now executes in the required unit lane (the `[juniper-cascor]` extra added to the CI install). Every wave-1 AND wave-2 unit is now `merged`, and all except CL2's live half are `verified` (CL2's runtime behavior gets its live pass with the next stack session). CI note for the fleet: canopy#453's Security Scans job red-flapped through the 2026-07-19/20 GitHub API outage because gitleaks-action enforces license validation when its user-lookup fails — a hardening candidate (fleet-wide). Remaining plan work: wave 3 (N3 — carrying the E-2-pinned active-run design, C5 surface consumption, the demo-FSM start-from-COMPLETED gap, and the start_fresh epoch-0 curiosity; N5; N6; C6 disposition per E-1's emission-exists verdict) and wave 4 (N7/N8/C7/N9/N10/E1).

**2026-07-20 addendum (2) — wave 3 delivered.** Three parallel agents shipped the canopy wave: **N6 merged** (canopy#458 — the S12 "10000 vs 12" root fix: the header's Epoch was showing completed training steps; now Step/Hidden-Units/true-Iteration/phase-qualified-Epoch per cascor#400's counter contract, kind-aware tiles). **N3 pr-open** (canopy#459, CI green — server-side `/api/train/restart` orchestration with the E-2 pin, Q3/Q4 confirm modal with the start-fresh toggle, all outcomes surfaced incl. the epoch-0 instant-convergence case; the demo FSM gains a terminal-state auto-reset mirroring cascor's engine — an intended BEHAVIOR CHANGE flagged for owner review; `start_fresh` reaches cascor via the client's `_post` as a documented seam until the client grows the field). **N5 pr-open** (canopy#460 — the PATCH-bounds mirror surfaced REAL bound divergences between canopy's own constants and cascor's request model, now drift-pinned; verbatim rejection toasts; C2a applied/skipped rendered end-to-end; WS leg gated on the CL2 liveness surface). **New unit N3b** (granular in-place modify in the restart modal) split out of N3 to avoid N5's in-flight apply contract; sequence after N5. **C6 dropped(satisfied)** per E-1's verdict. CI note: N3's adapter-level `juniper_cascor_client.constants` import exposed that the integration lane still resolved an ancient client — fixed in-branch by mirroring CL2's `[juniper-cascor]`-extra install (the lane-level dependency class to remember for N7/N8).

**2026-07-21 addendum — wave 3 complete.** N3b merged (canopy#462). Waves 1–3 of the roadmap are fully landed: every D/C/CL/N unit through N6 is `merged` (most `verified` live), C6 `dropped(satisfied)`, and the three experiments are `verified`/resolved. Remaining: wave 4 — N7 (dataset panel type-awareness, dep D1 ✓), N8 (WS-primary tiles/state per Q6, deps C6-verdict + CL2 ✓), C7 (expanded evaluation metrics, phased), N9 (metrics visualization overhaul, dep C7), N10 (workers tab), E1 (isolated-stack E2E checklist — content already queued from the E2E passes: the control-WS origin/allowlist pair, the data-venv bootstrap incl. [api] extra + observability deps, the CI-lane extras class, the CI-vs-local dataset-numerics divergence).

**2026-07-21 addendum (2) — wave-4 batch 1 delivered.** N7 (canopy#465), C7 phase-1 (cascor#419) and E1 (juniper-ml#686) all pr-open with local gates green (the three agents were cut mid-flight by an Anthropic session-limit reset and resumed from their transcripts without redone work). Remaining wave-4: N8 (WS-primary tiles/state — next dispatch), N9 (after C7 merges: renders the new scalar fields), N10 (worker-registration discovery pass first).

**2026-07-22 addendum — wave 4 delivered; the plan's full unit set is landed or pr-open.** N8 (canopy#472), N9 (canopy#471) and N10 (canopy#470) close the roadmap. ANOMALY (recorded, owner-flagged): during an Anthropic session-limit interruption, N9's and N10's feature code reached canopy main as DIRECT PUSHES (`a848cac`, `fa5f32a`) outside any PR — N9's push additionally corrupted a test into a collection-breaking form, redding main and every PR's merge-result CI; a pre-existing worker-route shape pin also went stale. Both agents verified the landed code matches their tested work and adapted with record PRs (the cascor#407 pattern); #470 aggregates all repairs (worker-pin alignment + N9's byte-identical module fix + changelog) — **merge #470 first** to un-red main, then #471 (clean identical-content merge), then #472. N8's report contributes two durable engineering findings: a Dash pitfall (a clientside-fed store as an interval callback's Input lets a chained no_update SKIP the poll entirely — the starvation class returns by construction; use a two-callback split) and the latent dead `state_change`-vs-`state` bridge handler that explains why ws-state-buffer never populated in the Phase-B era. MERGE ORDER: #470 → #471 → #472. After those merges every unit of this plan is `merged`/`verified`/`dropped(satisfied)` — the remediation roadmap is COMPLETE; residual follow-ups live in their row notes (cascor local-worker surface; plotter-selector gating; deploy data-service floor).

**2026-07-23 addendum — PLAN COMPLETE.** canopy#470/#471/#472 merged: every unit of this remediation plan across all four waves is now `merged` (waves 1–2 additionally live-`verified`; C6 `dropped(satisfied)`; the three experiments resolved). The arc, end to end: 5 wave-1 defect fixes → 8 wave-2 hardening units + 2 live experiments + a three-package PyPI deploy train (cascor-client 0.7.0, cascor 0.6.0, data 0.10.0) → 4 wave-3 UX/orchestration units → 6 wave-4 feature units — plus, along the way, three main-branch CI reds diagnosed and fixed (canopy UI leg, cascor quick-integration, canopy post-direct-push), two latent production bugs found by verification (demo-topology adapter import; service-mode snapshot local-stat), and two direct-push anomaly windows documented. Residual follow-ups (non-blocking, owned by their row notes): cascor local-worker surface for N10's full local/remote roster; canopy plotter-selector availability gating; deploy-repo juniper-data service floor; C7 phase-2 curve/explainability artifacts. This tracker is hereby CLOSED for wave work — future maintenance updates should note-reference this addendum.
