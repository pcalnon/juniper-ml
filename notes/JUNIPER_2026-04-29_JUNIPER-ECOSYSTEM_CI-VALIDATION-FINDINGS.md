...
Companion to `notes/JUNIPER_2026-04-29_JUNIPER-ECOSYSTEM_CI-VALIDATION-ROADMAP.md` Phase V1.
...
| V17 | juniper-data            | lockfile-update.yml | update                                              | secret missing (CROSS_REPO_DISPATCH_TOKEN)                         | P2       | G-CONFIG   | **fixed (verified)** | `CROSS_REPO_DISPATCH_TOKEN` provisioned and validated end-to-end 2026-05-02. See notes/JUNIPER_2026-05-02_JUNIPER-ECOSYSTEM_V17-V18-CROSS-REPO-DISPATCH-TOKEN-SETUP.md for the walkthrough. |
| V18 | juniper-canopy          | lockfile-update.yml | update                                              | secret missing (CROSS_REPO_DISPATCH_TOKEN)                         | P2       | G-CONFIG   | **fixed (verified)** | Same secret as V17; validated end-to-end 2026-05-02. |
...
|     |                         |                     |                                                     | JUNIPER_2026-05-25_JUNIPER-ECOSYSTEM_OUTSTANDING-DEVELOPMENT-ITEMS-V7-IMPLEMENTATION-ROADMAP.md |          |            |              | the missing design docs (JUNIPER_2026-05-02_JUNIPER-ECOSYSTEM_PHASE-6E-DESIGN.md, CAN_013_*.md)            |
|     |                         |                     |                                                     | → JUNIPER_2026-05-02_JUNIPER-ECOSYSTEM_PHASE-6E-DESIGN.md, JUNIPER_2026-04-29_JUNIPER-CASCOR_CAN-013-INTEGRATION-MODE-DESIGN.md, etc.)    |          |            |              | were landed via downstream commits                                    |
...
|     |                         |                     |                                                     |                                                                    |          |            |              | gate.                                                                |
| V34 | juniper-canopy          | ci.yml              | unit-tests (matrix; 4 legs)                         | (a) `Duplicated timeseries in CollectorRegistry` (prom metric       | P2       | G-CODE     | **fixed (verified)** | canopy `b33f81f` — (a) `_ensure_canopy_metrics` now adopts existing |
|     |                         |                     |                                                     | registered twice across test files);                                |          |            |              | collectors when a fixture has nulled the dict cache but REGISTRY    |
|     |                         |                     |                                                     | (b) `property 'running' of 'DemoMode' object has no setter`        |          |            |              | still has them (`_get_or_create` per-metric); (b) test was setting  |
|     |                         |                     |                                                     |                                                                    |          |            |              | the read-only `running` @property — replaced with `is_running`.      |
| V35 | juniper-cascor          | ci.yml              | unit-tests (matrix; 4 legs)                         | (a) `test_api_middleware.TestRequestBodyLimitMiddleware` chunked-   | P2       | G-CODE     | **fixed (verified)** | cascor `0a8bf7b` + `0dfed68` + `af19715` — (a) Content-Type header  |
|     |                         |                     |                                                     | body 422 ≠ 200; (b) `test_app_startup_tasks` log-capture            |          |            |              | added to chunked-body tests (FastAPI `payload: dict` only invokes   |
|     |                         |                     |                                                     | (assert 0 == 1); (c) `test_coordinator_health_monitor_race`         |          |            |              | JSON parsing when Content-Type is application/json); (b) test now   |
|     |                         |                     |                                                     | CONC-10 race on stale-worker dereg; (d)                            |          |            |              | monkey-patches `api.app.logger` with an isolated capture logger     |
|     |                         |                     |                                                     | `test_lifecycle_manager` epochs_max default 1e11 vs 200 drift      |          |            |              | (caplog fell over because something else in the suite mutates the   |
|     |                         |                     |                                                     | (e) `test_websocket_messages` `is`-vs-`==`                          |          |            |              | `juniper_cascor.api` logger before this test runs); (c) CONC-10     |
|     |                         |                     |                                                     |                                                                    |          |            |              | `get_next_assignment` checks registry presence under `_lock`; (d)   |
|     |                         |                     |                                                     |                                                                    |          |            |              | lifecycle training-limit defaults restored to spec; (e) two         |
|     |                         |                     |                                                     |                                                                    |          |            |              | identity assertions on `model_dump()` switched to value-equality.    |
| V36 | juniper-canopy          | ci.yml              | unit-tests (post-V34)                               | (a) `_HOT_CASCOR_PARAMS` count drift (added `output_epochs` in     | P2       | G-CODE     | **fixed (verified)** | canopy `8cc72d9` — (a) bumped expected count 11→12 reflecting       |
|     |                         |                     |                                                     | Phase 6E A-1, test still asserted 11);                              |          |            |              | `output_epochs` addition (`1542600`); (b) scaled TTL/sleep budget   |
|     |                         |                     |                                                     | (b) macOS-flaky CSRF TTL (`test_validate_refreshes_ttl` jitter)    |          |            |              | 5x to absorb macOS scheduling jitter on CI.                          |
| V37 | juniper-cascor          | ci.yml              | unit-tests (post-V35)                               | (a) `test_list_workers_serialization_fields` missing 3 R4.4/R4.4b   | P2       | G-CODE     | **fixed (verified)** | cascor `af19715` — (a) added 3 telemetry keys to expected_keys;    |
|     |                         |                     |                                                     | telemetry keys; (b) 3 `ws_heartbeat` tests count-drained 3 frames   |          |            |              | (b) replaced count-drain with skip-until-`ping` loop (budget=10),   |
|     |                         |                     |                                                     | but GAP-WS-16 added a 4th (`initial_metrics`) on /ws/training       |          |            |              | robust to additional initial frames being added later.               |
| V38 | juniper-cascor          | ci.yml              | unit-tests (post-V37)                               | (a) `test_grow_network_adds_hidden_unit` + (c) 3                    | P2       | G-CODE     | **fixed (verified)** | cascor `cd86a90` (V38b/d/e) + cascor PR #181 (V38a/V38c — Option E) — |
|     |                         |                     |                                                     | `test_fast_fit` tests: `grow_network` returns None candidate;       |          |            |              | `create=True` (Linux-only attribute); (d) `SharedMemory(track=False)`|
|     |                         |                     |                                                     | (b) `os.sched_getaffinity` macOS-only mocks;                        |          |            |              | guarded behind `sys.version_info >= (3, 13)` (real product fix —    |
|     |                         |                     |                                                     | (d) `SharedMemory(track=False)` py3.13+ kwarg in production code;   |          |            |              | py3.12 doesn't have the kwarg); (e) `test_spiral_check_coverage`    |
|     |                         |                     |                                                     | (e) `spiral_problem.check` archived to src/backups/                  |          |            |              | guarded with `pytest.importorskip` (V31 archived-module pattern).   |
|     |                         |                     |                                                     |                                                                    |          |            |              | V38a/V38c (4 flaky tests asserting hidden_units count) closed via   |
|     |                         |                     |                                                     |                                                                    |          |            |              | Option E in cascor PR #181 — retagged `@pytest.mark.integration`.   |
|     |                         |                     |                                                     |                                                                    |          |            |              | Phase A.2 diagnostic confirmed RC-4 (multiprocessing race) via      |
|     |                         |                     |                                                     |                                                                    |          |            |              | classic Heisenbug: `print(flush=True)` I/O sync makes the bug       |
|     |                         |                     |                                                     |                                                                    |          |            |              | disappear deterministically. Underlying race tracked separately.    |
