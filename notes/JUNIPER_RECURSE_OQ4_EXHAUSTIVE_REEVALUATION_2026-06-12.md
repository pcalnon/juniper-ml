...
1. **Default next development step: the cheap star-free path.** Adopt **P4-FIR + Δt-as-feature** as the temporal substrate — lowest integration cost, mini-batch-friendly, and sufficient for every dataset the *current* catalog plausibly targets (per the §5 operational-demands audit of all juniper-data generators). The ceiling is not binding today. **⚠️ Near-term only:** this default is acceptable *near-term*; the ceilinged star-free family will **not** meet **mid-term or long-term** requirements once a task genuinely needs modular / parity / group counting (or the platform's reasoning ambitions grow) — at which point a genuine ceiling-breaker becomes required (item 3; the **"P7"** design, §8 item 3, now under development).
...
4. **Correction applied (2026-06-13):** the model doc (`JUNIPER_RECURSE_MODEL_DESIGN_AND_PLAN_2026-05-31.md`) asserted ESN/LMU "does not share RCC's star-free ceiling" — a **category error** (ESN/LMU *are* star-free; Sarrof–Veitsman–Hahn 2024 + the measured ESN parity collapse contradict it). **Now retracted/corrected in place** to "fading-memory regression, not ceiling-breaking," with a RESOLVED callout added to §1.3 (the offending text was at the current line ~190, with related instances at ~153 / ~170 — the earlier ":173" reference had drifted).
