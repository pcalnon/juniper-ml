...
- **(a) Add the fixed-Δt negative control to the POC.** Extend `verify_delta_t_reference_code.py` with a `FixedStepLMUMemory` variant (bakes `Ā`, `B̄` at the *mean* Δt and applies them on the irregular grid) and assert it **fails** `e_irr < 3·e_reg + 0.02` while the variable-step memory passes. Converts the model's central claim from analytic assertion to measurement. *(Small, high-value; ad-hoc POC, `util/ad-hoc/`.)*
