...
| **v2-ship — Regenerate + PR** | ☐ in_progress | 2026-05-14 | — | (this commit) | v2 corpus: 2,120 candidates → 1,801 entries (319 dedup'd). 768 net new requirements vs v1's 1,033. |
...
### New issues discovered during v2

| # | Issue | Severity | Suggested resolution |
|---|---|---|---|
| 8 | **Phase-3c agents truncated on "elephant" files within their slice.** 3c-3a processed 8/51 (canopy elephant: PHASE_4_CANOPY_CASCOR synthesis at 1251 LOC); 3c-3b processed 8/42 (cascor elephant: INTEGRATION_ROADMAP-01.md). Required 3c-3a-2 and 3c-3b-2 follow-up agents; even those left ~30 canopy files unprocessed. Pattern: when one file in scope dominates density, agents spend budget there and skip the rest. | Medium | v3 should pre-scan slice for "elephant" files (any single file >30% of slice score) and split them out into their own dedicated agent. |
| 9 | **3c-3b-2 invented 6 invalid category codes** (AR/BG/CI/CL/SE/TI). Auto-remapped via blanket rules (AR→ARCH, BG→TRAIN, CI→TEST, CL→TOOL, SE→API, TI→TEST). The blanket remap is coarse — some `BG` entries probably belong in TEST not TRAIN; some `SE` entries probably are SEC not API. | Low | v3 should add a category-code validation gate in the agent brief that fails YAML emission if codes aren't from the locked enum, forcing the agent to retry with a valid code. |
| 10 | **Cross-repo content dedup needs fuzzy match.** v2-3 used exact-brief + same-basename-tuple which caught 0 cases despite 60 entries citing `FINAL_CANOPY_CASCOR_CONNECTION_ANALYSIS.md` across 2 owners. But blunt fuzzy match risks over-merging legitimately separate per-repo artifacts (e.g., `AGENTS_MD_AUDIT_ANALYSIS_2026-04-02.md` in dcl and ccl are different audits). | Medium | v3 should use fuzzy match (TF-IDF or embedding cosine sim) WITH a manual review queue: auto-merge only when similarity > T and basename + first-N-chars-of-brief both match. |
| 11 | **Score 1-9 long tail (~245 files) unprocessed.** Many are boilerplate or pure cleanup notes; expected per-file yield is 0-2 entries. | Low (cost > benefit) | Skip unless coverage becomes a stated requirement. |
| 12 | **ARCH re-bucket rules are heuristic and may misclassify.** Each rule is a regex on brief+detail keywords; a brief about "WebSocket security" matches WS first (since WS rule is earlier in the rule list). | Low | v3 may add explicit priority weighting or LLM-based per-entry classification. |
