...
| CAN-015  | Training Metrics  | Snapshot operations: Restore / Replay / Resume / Retrain (was: replay with live tuning) | P3 | 🚧 **Phase 6E Sprint B (2026-05-01 design)** — fan-out into 6 PRs: CAN-015a (Retrain), CAN-015b (Resume), CAN-015c (Replay infra V1: metrics + topology), CAN-015d (Restore enrichment + Investigating FSM), CAN-015e (canopy entry points), CAN-015f (canopy replay player UI). Full design in [`PHASE_6E_SPRINT_B_DESIGN.md`](PHASE_6E_SPRINT_B_DESIGN.md). Two future items deferred to dedicated post-Phase-6E sprints: **CAN-015g** (Replay V2 with per-epoch weight history — requires snapshot-format extension; risk: file-size inflation), **CAN-015h** (Restore weight + topology editing endpoints — needs careful validation/cascade-rebuild semantics). |
