...
| 6E    | CAS-002..009 + 6 deferred CAN items     | 9×S-M + 1×XL | CasCor algorithm enhancements + cross-repo CAN-* | 🔴 Outstanding — design doc landed 2026-04-29 ([`notes/PHASE_6E_DESIGN.md`](PHASE_6E_DESIGN.md)). Scope reframe: 3 of 6 deferred CAN items are **wire-through, not algorithm-blocked** (cascor's optimizer factory, activation hook, and snapshot training-state capture all already exist; only the API + UI surfaces are missing). Plan: 9 small/medium PRs cover CAS-002/003/005/006, ENH-006/007, CAN-010/011/013-narrow/014/015. **One genuinely XL item** — multi-network architecture (CAS-008/CAS-009/CAN-021) — gets its own design pass before coding. Was 14×M-XL; now 9×S-M + 1×XL. |
...
**Track 6 progress (2026-04-29 — design pass)**: Phases 6A, 6B, 6C, 6D complete (4/6). 6F drained (only conditional Phase E..H backpressure pump tasks left). **6E design doc landed**: [`notes/PHASE_6E_DESIGN.md`](PHASE_6E_DESIGN.md) reframes the scope. Three "blocked" CAN items (CAN-010 optimizer, CAN-011 activation, CAN-014 snapshot tuning capture) turn out to be **wire-through, not algorithm-blocked** — cascor's infrastructure already supports them. Plan: 9 small/medium PRs (Sprint A: 5 wire-through wins + auto-snap; Sprint B: snapshot-replay loop + integration-mode picker; Sprint C: CAS-005 shared-code cleanup) plus 1 XL design pass for the multi-network refactor (CAS-008/CAS-009/CAN-021). Adjacent **Network Evolution** ([juniper-canopy #203](https://github.com/pcalnon/juniper-canopy/pull/203)) already covers part of CAN-021's "see multiple network states" intent.
