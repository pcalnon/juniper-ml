...
**Validated** by consensus in two rounds of three independent lanes, archived as `reports/2026-09-24_defect-register-round-42/handoff-2fba4397-round{1,2}-lane{F-reprobe,O-amputation,P-fresh-session}.md`.
...
**Superseded if consolidated:** the owner asked for this handoff to be merged with two others (Remaining 0).
- Look for the consolidated handoff in `prompts/thread-handoff_automated-prompts/` on `origin/main`, and in open juniper-ml PRs.
- If it exists, work from it instead of this file.
...
2. **Treat the executor as ALIVE** (In flight 1) while `ListAgents` lists "defect reg [24f8d8]" in any state but offline: a live parent can resume a stopped executor. While it is alive, do not touch the juniper-data worktree, do not launch a second executor, and never ask the owner to close that session, which would kill the executor.
3. **Re-route the peer lane.** It is handing off too, and its own handoff says never to message its old session, "defect reg [042116]" (session `bc31e993`). It tells ITS successor to message "defect reg [24f8d8]". Unless the consolidated handoff has merged the two lanes into one:
   - message the peer's SUCCESSOR: the other live session working that handoff, found with `ListAgents`; ask the owner if you cannot tell. Give it your name and `[ref]`, and say you replace [24f8d8];
...
4. **Ship the uncommitted files** (Git status) only once "defect reg [24f8d8]" is gone AND the executor has finished. Until then that session is still writing here.
   - First re-copy the PR draft (Appendix C).
   - Check that no open juniper-ml PR carries these paths.
...
- **juniper-ml#2089 OPEN**, head `a2fa3ad8`, not armed. It preserves the 129 Python probe scripts of this session's six lanes, off tmpfs, in `util/ad-hoc/2026-09-24_round42_probes/<report stem>/`, as #2081 did for `8f86dec2`'s. The 67 shell runners are not kept (README).
...
1. **Executor `a46e715a6801b98ca`** is fixing every finding of both data lanes (Appendix C).
...
0. **Consolidate** (the owner's instruction, 2026-09-24 about 23:25Z): merge three handoffs into one, validated by consensus with no task or context lost.
   - The three: this file; the predecessor (on `main`); and the peer's `HANDOFF_2026-09-24_round-42-follow-up-lane-four-prs-await-validation-and-the-observability-release.md`, untracked in `/home/pcalnon/Development/python/Juniper/juniper-ml/.claude/worktrees/happy-skipping-hollerith/prompts/thread-handoff_automated-prompts/`. Its PR will be on branch `docs/handoff-round42-followup-lane`.
   - No consolidated draft exists yet. "defect reg [24f8d8]" agreed the split with the peer at 23:35:52Z: the `surrogatepass` marker and C-A…D-G are this lane's. It asked for the peer's final version at 23:34:29Z.
   - Reconcile:
     - the peer's "no forward has been confirmed" for the canopy items is stale (Appendix A);
     - the peer's README rows now live on #2089;
     - the two handoffs close `APD-ECO-013` under different conditions (Appendix A).
...
6. **Verify on the real services, then file.**
   - (a) A 422 that echoes the input through Starlette's `JSONResponse` (`ensure_ascii=False`) raises `UnicodeEncodeError` on a lone surrogate. That is a `ValueError`, so the outcome depends on the app's handlers (round-2 lane F, measured in-process):
     ...
     - juniper-data (`juniper_data/api/app.py:187-212`, with its `ValueError` handler at `:214`): **400 `{"detail":"Invalid request parameters"}`**. The 422's per-field detail is silently lost, and no Sentry event is sent.
     - cascor (`src/api/app.py:856`, `ValueError` handler `:916`): expected to be the same, but not run.
...
- **Routing:** "canopy combined [577a1c]" and "containers [2703c8]". The canopy E2E session has exited.
...
- **Sentry:** set every DSN variable to `""` in every test run and server: `SENTRY_SDK_DSN`, `SENTRY_DSN`, `JUNIPER_CASCOR_SENTRY_DSN`, `JUNIPER_DATA_SENTRY_DSN`, `CANOPY_SENTRY_DSN`, `JUNIPER_CANOPY_SENTRY_DSN`. Set them empty, never unset: `load_dotenv` re-injects an unset one. Never use port 8100, 8201 or 8050.
...
- **The worktree-isolation guard refuses:**
...
```
git status --short                     # 17 entries at handoff; see Git status
...
gh pr list --repo pcalnon/juniper-ml --head docs/handoff-round42-followup-lane --state all   # none at handoff
...
python3 util/ad-hoc/2026-09-24_archive_round42_reports.py --check | grep -c 'OK    '   # 42 here; 34 in a fresh checkout until the next PR ships its 8 reports
...
- **For the next juniper-ml PR (nothing else holds these):**
  ...
  - In `reports/2026-09-24_defect-register-round-42/`, all untracked:
    ...
    - the six `handoff-2fba4397-round{1,2}-*.md`;
...
- **`APD-ECO-013` (S).** A non-ASCII `X-API-Key` makes the `str` `compare_digest` raise; the 500 goes to Sentry, and its local-variable capture records the real key.
  ...
  - **When it closes differs between the two handoffs. The consolidation must choose one:**
    - this one: once data#440 and cascor#689 merge, #685's validation holds, and the peer's F2 and F3 are fixed;
    - the peer's (its line 131): once the three bytes-compare PRs merge and validate.
  ...
  - **Releases.** The frame-locals fix reaches no running service until juniper-observability is released and every consumer's `==0.4.0` lock moves:
    ...
    - The peer's handoff lists further `<0.8.0` service-core caps; the consolidation must keep them.
...
- The durable copy is `reports/2026-09-24_defect-register-round-42/data438-fixforward-pr-draft.md`: `# <title>`, a blank line, then the body.
  ...
  - Re-copy it with a script once the executor finishes: `"# " + title + "\n\n" + body`.
...
- **Canopy's `.encode` sits only in `_compare_bytes`** (`src/security.py:42-52`), which `:300` also uses. A revert to `presented = api_key` that keeps `_compare_bytes` passes even the literal pair. The peer's F3 spy test, with its bytes-argument check, catches it.
...
- **PR CI will not catch a premature marker.** `ci.yml:500` runs the whole file but skips every fork site that has no siblings, so it merges green and fails the next weekly `docs-full-check` run on `main`. Test before pushing with `JUNIPER_DRIFT_TEST_FORCE_LOCAL=1` and the siblings at `origin/main`.
- **The same change:**
  - corrects `tests/test_service_fork_drift.py:205-209`, "no behavioural test can tell them apart", which the peer's F8 refuted;
