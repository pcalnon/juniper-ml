#!/usr/bin/env python3
"""Close juniper-ml's [Unreleased] into [0.10.0], and re-home #2002's misfiled bullets, before the cut.
...
Why this exists
---------------
The ceremony renders the Release body -- which cannot be re-cut -- from ``## [0.10.0]`` alone. The
tag it cuts carries every commit on ``main``. So every bullet describing a commit in the tag window
must sit in ``[0.10.0]`` at the moment of the cut. Two ways it did not:
...
* **juniper-ml#2002** merged at 09:44:48Z, 22 minutes AFTER ``v0.9.0`` was tagged, and its four
  ...
  only ``[0.9.0]`` bullets absent from ``notes/releases/RELEASE_NOTES_v0.9.0.md`` (4 of 22). The
  CHANGELOG therefore claims 0.9.0 shipped D2 and D6; the 0.10.0 notes would never mention them.
* **Every PR merged after the 0.10.0 bump (#2033)** filed under ``[Unreleased]`` -- correct
  normally, wrong here, because a Release tags the branch HEAD. #2036 did; then, while the first
  fold PR (#2040) was in CI, #2047 added six more and the fold conflicted.

v0.1.0 of this script moved #2036's bullet by name. v0.2.0 closes ``[Unreleased]`` WHOLE -- a
version move -- because on a lane this busy, naming the bullets loses the race to the next merge.
...
unchanged. Bullets are moved verbatim (modulo trailing whitespace, which the parser drops).
...
#: #2002's bullets under the released [0.9.0]: first-line prefix -> the category it belongs to.
_FROM_090 = [
...
def _all_bullets(text: str) -> list:
    out = [b for bs in ceremony.notes_render.parse_unreleased(text).values() for b in bs]
    for version in ("0.10.0", "0.9.0"):
        out += [b for bs in ceremony.changelog_version_section(text, version).values() for b in bs]
    return sorted(out)
...
def main() -> int:
    ...
    groups: dict = {}  # category -> list of verbatim bullet blocks, in build order

    def add(category: str, block: str) -> None:
        groups.setdefault(category, []).append(block)

    # 1. What [0.10.0] already holds leads each category: it is the release's own point.
    for category, bullets in ceremony.changelog_version_section(text, "0.10.0").items():
        for b in bullets:
            add(category, "- " + b)
    # 2. Everything under [Unreleased], in file order -- a version move, not a pick-list.
    unreleased = ceremony.notes_render.parse_unreleased(text)
    n_unreleased = sum(len(v) for v in unreleased.values())
    for category, bullets in unreleased.items():
        for b in bullets:
            add(category, "- " + b)
    # 3. #2002's four bullets, cut verbatim out of the released [0.9.0].
    drop = [(i_unr + 1, i_new), (i_new + 1, i_090)]
    for category, prefix in _FROM_090:
        span = _take(lines, i_090, i_080, prefix)
        add(category, "\n".join(lines[span[0] : span[1]]).rstrip())
        drop.append(span)

    order = [c for c in _CATEGORY_ORDER if c in groups] + [c for c in groups if c not in _CATEGORY_ORDER]
    new_body = [""]
    ...
    left_in_090 = [b for bs in ceremony.changelog_version_section(result, "0.9.0").values() for b in bs]
    ...
    before, after = _all_bullets(text), _all_bullets(result)
    if before != after:
    ...
    if n_new != expected:
        raise SystemExit(f"ERROR: [0.10.0] should hold {expected} bullets after the fold, holds {n_new}")
    ...
    print(f"[Unreleased]: {n_unreleased} bullet(s) closed into [0.10.0]; [0.9.0]: 4 re-homed, {len(left_in_090)} left, all in RELEASE_NOTES_v0.9.0.md")
    print(f"[0.10.0]: {n_new} bullets -- " + ", ".join(f"{c} {len(groups[c])}" for c in order))
