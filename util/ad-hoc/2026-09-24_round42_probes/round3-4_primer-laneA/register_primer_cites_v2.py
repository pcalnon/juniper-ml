...
import os
wt_text = Path(os.environ.get("WT_PRIMER_OVERRIDE", str(WT / PRIMER_REL))).read_text(encoding="utf-8")
...
for regline, rid, cell in primer_fields:
    ...
    if os.environ.get("SHOW_PARSE") and nums:
        print(f"  R{regline} {rid}: {cell[:70]!r} -> {sorted(set(nums))[:12]}")
    if not nums:
