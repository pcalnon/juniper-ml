...
for k in [k for k in os.environ if k.startswith(("JUNIPER_CANOPY_", "CANOPY_", "JUNIPER_CASCOR", "JUNIPER_DATA", "CASCOR_"))]:
    del os.environ[k]
os.environ.update({"JUNIPER_CANOPY_DEMO_MODE": "true", "JUNIPER_CANOPY_JUNIPER_DATA_URL": "http://127.0.0.1:1", "CANOPY_API_KEY": "   "})
...
extra = sorted(p for m, p in pget if p in ("/docs", "/redoc", "/openapi.json", "/docs/oauth2-redirect"))
print(f"docs routes among gated GETs: {extra}")
