#!/usr/bin/env python
"""F-CANOPY-054 round 3, Lane B2: v2 (85415f3c) vs v3 (a967a5bd), paired -- is ONE pause click applied twice?

Round-3 copy of util/ad-hoc/2026-09-23_f054_r2_laneB2_double_apply_cleanroom.py (kept unchanged as the
provenance of the archived round-2 transcripts). Changes: a git ref PER ARM, so v2 and v3 arms alternate
under one lever; and the wrapper now separates a RACE RUN (a run whose trigger names the play button while
its count was already applied -- the renderer delivered the sequence) from a DOUBLE TOGGLE (such a run that
changed the mode). v3 must show race runs with zero toggles; v2 toggles on them.
...
Arm spec: name:K:speed[:sat_ms_per_50ms[:fan[:delay0_ms[:git_ref]]]]. delay0 > 0 is a REACHABILITY lever, not a rate model. fan is each poller's downstream fan-out (default 3,
...
def load_js(ref):
    ...
    assert "state.clicks = seen;" in out["REPLAY_CONTROLS_JS"], f"{ref} does not count clicks (v1?)"
    out["ref"] = ref
    return out
...
async def run_arm(pw, name, k, speed, sat, trials, latency, js, seed, fan=3, delay0=0):
    ...
    for t in range(trials):
        ...
        doubles = [r for r in runs if "replay-play.n_clicks" in r["trig"] and r["play"] is not None and r["play"] == r["seenPlay"]]  # RACE RUNS
        toggles = [r for r in doubles if r["modeOut"] not in ("NU", r["modeIn"])]  # a race run that CHANGED the mode
        by_count = [r for r in runs if "replay-play.n_clicks" not in r["trig"] and (r["play"] or 0) > (r["seenPlay"] or 0) and r["modeOut"] != "NU"]
        rec = {"trial": t, "verdict": verdict, "pause_ms": (paused_at[0] - t_click) if paused_at else None, "undone_ms": (replayed[0][0] - t_click) if replayed else None, "double_runs": doubles, "double_toggles": toggles, "pause_by_count_runs": by_count, "runs": runs}
        results.append(rec)
        print(f"  [{name}] trial {t}: {verdict} pause_ms={rec['pause_ms']} undone_ms={rec['undone_ms']} by_count={len(by_count)} race_runs={len(doubles)} toggles={len(toggles)} runs={[ (r['trig'], r['play'], r['seenPlay'], r['modeIn'], r['modeOut']) for r in runs[:4]]}", flush=True)
    stats = await page.evaluate("() => ({n: window.__rec.notifies, full: window.__rec.poolFull})")
    await browser.close()
    held = [r for r in results if r["verdict"] == "HELD"]
    summ = {
        "arm": name, "ref": ref, "K": k, "speed": speed, "sat_ms_per_50": sat, "fan": fan, "delay0_ms": delay0, "trials": trials, "latency_s": latency,
        "verdicts": {v: sum(1 for r in results if r["verdict"] == v) for v in sorted({r["verdict"] for r in results})},
        "trials_with_race_run": sum(1 for r in results if r.get("double_runs")),
        "trials_with_double_toggle": sum(1 for r in results if r.get("double_toggles")),
...
async def main_async(a):
    ...
    cache = {}
    out = {"probe": Path(__file__).name, "latency_s": a.latency, "trials": a.trials, "arms": {}}
    async with async_playwright() as pw:  # ONE browser at a time: arms run sequentially
        for spec in a.arms.split(","):
            ...
            ref = rest[3] if len(rest) > 3 else a.ref
            if ref not in cache:
                cache[ref] = load_js(ref)
            print(f"arm {name}: ref={ref} K={k} speed={speed} sat={sat}ms/50ms fan={fan} delay0={delay0}ms L={a.latency}s", flush=True)
            out["arms"][name] = await run_arm(pw, name, int(k), speed, sat, a.trials, a.latency, cache[ref], a.seed, fan, delay0)
