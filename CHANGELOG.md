...
- **CW-05 candidate-core documentation refresh**: platform-level docs now surface the new independently published `juniper-cascor-core` package alongside the existing sibling packages. `README.md`, `docs/QUICK_START.md`, `docs/REFERENCE.md`, and `docs/DOCUMENTATION_OVERVIEW.md` document the direct-install status, top-level import contract (`candidate_unit`, `utils`, `log_config`, `cascor_constants`), worker activation tuple compatibility, exported remote collection timeout constants, deployment-agnostic logging env vars, drift-guard expectations, and `juniper-cascor-core-v*` publish workflow. The docs intentionally state that the package is not yet part of the `juniper-ml` extras surface; worker adoption remains the follow-up wave.
