...
- **R5: new `[recurrence]` extra — the Δt-native recurrence stack.** Adds a dedicated `recurrence` optional-dependency group pinning `juniper-recurrence-model>=0.1.0,<0.2.0` (the closed-form variable-Δt LMU regressor on the `juniper-model-core` `TrainableModel` seam) and `juniper-recurrence>=0.1.0,<0.2.0` (the FastAPI/CLI application that wraps it), now that both are published to PyPI (WS-4 / WS-4b). `[all]` aggregates it, so `pip install juniper-ml[recurrence]` / `[all]` resolves the recurrence model + app. The matching lint contract in `tests/test_pyproject_extras.py` updates in lockstep (RK-11); the `AGENTS.md` / `README.md` / `docs/QUICK_START.md` / `docs/REFERENCE.md` extras tables pick up the new row.
